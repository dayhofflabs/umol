...
/// fields are private so metadata cannot drift onto a different molecule.
...
impl MoleculeDsl {
    /// Pair a molecule with coherent surface metadata.
    pub fn new(molecule: Molecule, metadata: MoleculeMetadata) -> Result<Self, MetadataError> {
        ...
        Ok(Self::from_parts(molecule, metadata))
    }

    fn from_parts(molecule: Molecule, metadata: MoleculeMetadata) -> Self {
        Self { molecule, metadata }
    }
    ...
    pub fn into_parts(self) -> (Molecule, MoleculeMetadata) {
        (self.molecule, self.metadata)
    }
}

impl PartialEq for MoleculeDsl {
    fn eq(&self, other: &Self) -> bool {
        self.molecule == other.molecule && self.metadata == other.metadata
    }
}
...
impl<'de> FromEdn<'de> for MoleculeDsl {
    fn from_edn(edn: &Edn<'de>) -> Result<Self, DeError> {
        ...
        let (molecule, context) = input
            .into_ir()
            .map_err(|e| DeError::Custom(e.to_string()))?;
        Ok(MoleculeDsl::from_parts(molecule, context.into_metadata()))
    }
    ...
    fn from_edn_str(input: &'de str) -> Result<Self, EdnError> {
        ...
        de.expect_eof()?;
        let (molecule, context) = mi.into_ir().map_err(|e| DeError::Custom(e.to_string()))?;
        Ok(MoleculeDsl::from_parts(molecule, context.into_metadata()))
    }
}
...
/// refs (no entity keywords, no aliases) since the molecule carries no metadata.
        ...
        b'{' => {
            ...
            read_map(de, |de, key| {
                match key {
                    ...
                    "attrs" => bond = Some(read_bond_dsl(de)?),
            ...
            Ok(BondEntryInput {
                ...
                first: a,
                second: b,
                bond: bond.ok_or_else(|| missing("attrs", "bond-entry"))?,
            })
        }
...
pub(super) fn read_dative_bond_entry(
    ...
    read_map(de, |de, key| {
        match key {
            ...
            "attrs" => bond = Some(read_dative_dsl(de)?),
            _ => de.read_skip_value()?,
        }
        Ok(())
    })?;
    Ok(DativeBondEntryInput {
        ...
        bond: bond.ok_or_else(|| missing("attrs", "dative-bond-entry"))?,
    })
}
...
pub(super) fn read_aromatic_system_entry(
    ...
    let system = system.ok_or_else(|| missing("attrs", "aromatic-system-entry"))?;
...
pub(super) fn read_multicenter_bond_entry(
    ...
    let bond = bond.ok_or_else(|| missing("attrs", "multicenter-bond-entry"))?;
...
pub(super) fn read_noncovalent_bond_entry(
    ...
    Ok(NoncovalentBondEntryInput {
        ...
        first: a,
        second: b,
        bond: bond.ok_or_else(|| missing("attrs", "noncovalent-bond-entry"))?,
    })
}
...
pub(super) fn read_stereo_atom_entry(
    ...
    read_map(de, |de, key| {
        match key {
            ...
            "attrs" => stereo = Some(read_stereo_atom_dsl(de)?),
            _ => de.read_skip_value()?,
        }
        Ok(())
    })?;
    Ok(StereoAtomEntryInput {
        ...
        stereo: stereo.ok_or_else(|| missing("attrs", "stereo-atom-entry"))?,
    })
}
...
pub(super) fn read_stereo_bond_entry(
    ...
    read_map(de, |de, key| {
        match key {
            ...
            "ligands" => ligands = Some(read_vec(de, read_stereo_ligand)?),
            "attrs" => stereo = Some(read_stereo_bond_dsl(de)?),
            _ => de.read_skip_value()?,
        }
        Ok(())
    })?;
    Ok(StereoBondEntryInput {
        ...
        stereo: stereo.ok_or_else(|| missing("attrs", "stereo-bond-entry"))?,
    })
}
...
impl ToEdn for MoleculeDsl {
    fn to_edn(&self) -> Edn<'static> {
        render_molecule_edn(&self.molecule, &self.metadata)
    }
}
...
pub(super) fn render_molecule_edn(molecule: &Molecule, meta: &MoleculeMetadata) -> Edn<'static> {
    let mut map = EdnMap::with_capacity(8);
    map.insert(Edn::keyword("atoms"), render_atoms(molecule, meta));
    map.insert(Edn::keyword("bonds"), render_bonds(molecule, meta));
    if molecule.dative_bonds().count() > 0 {
        map.insert(Edn::keyword("dative-bonds"), render_dative(molecule, meta));
    }
    if molecule.aromatic_systems().count() > 0 {
        map.insert(
            Edn::keyword("aromatic-systems"),
            render_aromatic(molecule, meta),
        );
    }
    if molecule.multicenter_bonds().count() > 0 {
        map.insert(
            Edn::keyword("multicenter-bonds"),
            render_multicenter(molecule, meta),
        );
    }
    if molecule.noncovalent_bonds().count() > 0 {
        map.insert(
            Edn::keyword("noncovalent-bonds"),
            render_noncovalent(molecule, meta),
        );
    }
    if molecule.stereo_atoms().count() > 0 {
        map.insert(
            Edn::keyword("stereo-atoms"),
            render_stereo_atoms(molecule, meta),
        );
    }
    if molecule.stereo_bonds().count() > 0 {
        map.insert(
            Edn::keyword("stereo-bonds"),
            render_stereo_bonds(molecule, meta),
        );
    }
    if meta.iter_atom_aliases().len() != 0 {
        map.insert(Edn::keyword("atom-aliases"), render_atom_aliases(meta));
    }
    let constraints_dsl = ConstraintsDsl::from_ir(molecule.constraints(), meta)
        .expect("ConstraintsDsl::from_ir is infallible for well-formed graph IR");
...
fn render_atoms(molecule: &Molecule, meta: &MoleculeMetadata) -> Edn<'static> {
...
/// A bond entry — `[a b attributes]`, or `{:id … :atoms [a b] :attrs attributes}` when the bond
/// has a keyword. `attributes_edn` is the already-rendered `:attrs` value: one bond DSL value for a
/// molecule, or a `[left right]` vector or operation-wrapped map for a span entry.
pub(super) fn render_bond_entry(
    ...
    match meta.keyword(Entity::Bond(id)) {
        Some(name) => {
            ...
            m.insert(Edn::keyword("attrs"), attributes_edn);
            Edn::Map(m)
        }
        None => Edn::Vector(vec![first, second, attributes_edn].into()),
    }
}

fn render_bonds(molecule: &Molecule, meta: &MoleculeMetadata) -> Edn<'static> {
    let entries: Vec<Edn<'static>> = molecule
        .bonds()
        .iter()
        .map(|view| {
...
// Overlay entries: `render_<entity>_entry` builds one entry map (`:id`? + participants + `:attrs`),
// with `:attrs` = the caller-supplied `attributes_edn`. Molecule and reaction-delta rendering pass an
...
fn render_dative(molecule: &Molecule, meta: &MoleculeMetadata) -> Edn<'static> {
    let entries: Vec<Edn<'static>> = molecule
        .dative_bonds()
        .iter()
        .map(|view| {
...
fn render_aromatic(molecule: &Molecule, meta: &MoleculeMetadata) -> Edn<'static> {
    let entries: Vec<Edn<'static>> = molecule
        .aromatic_systems()
        .iter()
        .map(|view| {
...
fn render_multicenter(molecule: &Molecule, meta: &MoleculeMetadata) -> Edn<'static> {
    let entries: Vec<Edn<'static>> = molecule
        .multicenter_bonds()
        .iter()
        .map(|view| {
...
fn render_noncovalent(molecule: &Molecule, meta: &MoleculeMetadata) -> Edn<'static> {
    let entries: Vec<Edn<'static>> = molecule
        .noncovalent_bonds()
        .iter()
        .map(|view| {
...
fn render_stereo_atoms(molecule: &Molecule, meta: &MoleculeMetadata) -> Edn<'static> {
    let entries: Vec<Edn<'static>> = molecule
        .stereo_atoms()
        .iter()
        .map(|view| {
...
fn render_stereo_bonds(molecule: &Molecule, meta: &MoleculeMetadata) -> Edn<'static> {
    let entries: Vec<Edn<'static>> = molecule
        .stereo_bonds()
        .iter()
        .map(|view| {
    ...
    match edn {
        ...
        Edn::Map(m) => {
            ...
            Ok(BondEntryInput {
                ...
                first: a,
                second: b,
                bond: BondDsl::from_edn(required_key(m, "attrs", "bond-entry")?)?,
            })
        }
        other => Err(DeError::TypeMismatch {
            expected: "bond-entry map or 3-vec",
...
pub(super) fn parse_dative_bond_entry(edn: &Edn<'_>) -> Result<DativeBondEntryInput, DeError> {
    ...
    Ok(DativeBondEntryInput {
        ...
        bond: DativeBondDsl::from_edn(required_key(m, "attrs", "dative-bond-entry")?)?,
    })
}
...
pub(super) fn parse_aromatic_system_entry(
    ...
    let system = AromaticSystemDsl::from_edn(required_key(m, "attrs", "aromatic-system-entry")?)?;
    Ok(AromaticSystemEntryInput {
        keyword: optional_id_keyword(m)?,
...
pub(super) fn parse_multicenter_bond_entry(
    ...
    let bond = MulticenterBondDsl::from_edn(required_key(m, "attrs", "multicenter-bond-entry")?)?;
    Ok(MulticenterBondEntryInput {
        keyword: optional_id_keyword(m)?,
...
pub(super) fn parse_noncovalent_bond_entry(
    ...
    Ok(NoncovalentBondEntryInput {
        ...
        first: a,
        second: b,
        bond: NoncovalentBondDsl::from_edn(required_key(m, "attrs", "noncovalent-bond-entry")?)?,
    })
}
...
pub(super) fn parse_stereo_atom_entry(edn: &Edn<'_>) -> Result<StereoAtomEntryInput, DeError> {
    ...
    Ok(StereoAtomEntryInput {
        ...
        stereo: StereoAtomDsl::from_edn(required_key(m, "attrs", "stereo-atom-entry")?)?,
    })
}
...
pub(super) fn parse_stereo_bond_entry(edn: &Edn<'_>) -> Result<StereoBondEntryInput, DeError> {
    ...
    Ok(StereoBondEntryInput {
        ...
        stereo: StereoBondDsl::from_edn(required_key(m, "attrs", "stereo-bond-entry")?)?,
    })
}
