...
pub enum RelationalConstraint {
    ...
    DativeBondAllDonors {
        bond: DativeBondId,
        predicate: Box<AtomConstraintForm>,
    },
    ...
    NoncovalentBondEndsSatisfy {
        bond: NoncovalentBondId,
        predicates: [Box<AtomConstraintForm>; 2],
    },
    ...
    pub fn compact(self, compaction: &MoleculeCompaction) -> Option<Self> {
        Some(match self {
            ...
            Self::DativeBondDonor { bond, atom } => Self::DativeBondDonor {
                bond: compaction.compact_dative_bond(bond)?,
                atom: compaction.compact_atom(atom)?,
            },
            ...
            Self::DativeBondParallels { dative, parallel } => Self::DativeBondParallels {
                dative: compaction.compact_dative_bond(dative)?,
                parallel: compaction.compact_bond(parallel)?,
            },
            ...
            Self::AromaticSystemContains { system, atom } => Self::AromaticSystemContains {
                system: compaction.compact_aromatic_system(system)?,
                atom: compaction.compact_atom(atom)?,
            },
            ...
            Self::MulticenterBondContains { bond, atom } => Self::MulticenterBondContains {
                bond: compaction.compact_multicenter_bond(bond)?,
                atom: compaction.compact_atom(atom)?,
            },
            ...
            Self::NoncovalentBondEnds { bond, atoms } => {
                let bond = compaction.compact_noncovalent_bond(bond)?;
                let [a, b] = atoms;
                Self::NoncovalentBondEnds {
                    bond,
                    atoms: [compaction.compact_atom(a)?, compaction.compact_atom(b)?],
                }
            }
            Self::NoncovalentBondContains { bond, atom } => Self::NoncovalentBondContains {
                bond: compaction.compact_noncovalent_bond(bond)?,
                atom: compaction.compact_atom(atom)?,
            },
            ...
            Self::StereoAtomSite { stereo_atom, atom } => Self::StereoAtomSite {
                stereo_atom: compaction.compact_stereo_atom(stereo_atom)?,
                atom: compaction.compact_atom(atom)?,
            },
            ...
            Self::StereoBondSite { stereo_bond, bond } => Self::StereoBondSite {
                stereo_bond: compaction.compact_stereo_bond(stereo_bond)?,
                bond: compaction.compact_bond(bond)?,
            },
...
impl FrameTransport for RelationalConstraint {
    ...
    fn reframe_by(self, actions: &Self::Action) -> Option<Self> {
        Some(match self {
            Self::NoncovalentBondEndsSatisfy { bond, predicates } => {
                let action = actions.noncovalent_bonds().action(bond)?;
...
impl Normalize for RelationalConstraint {
    ...
    fn normalize(self) -> Result<Self, Contradiction> {
        let normalize_atoms = |mut atoms: Vec<AtomId>| {
            atoms.sort_unstable();
            atoms.dedup();
            atoms
        };
        Ok(match self {
            ...
            Self::NoncovalentBondEnds { bond, mut atoms } => {
                atoms.sort_unstable();
                Self::NoncovalentBondEnds { bond, atoms }
            }
            Self::StereoAtomAllLigands {
...
mod tests {
    ...
    #[fixture]
    fn noncovalent_frame_actions() -> OverlayFrameActions {
        OverlayFrameActions::from_families(
            ...
            StereoAtomFrameActions::from_dense(vec![]).expect("actions are admissible"),
    ...
    fn val_pred() -> Box<AtomConstraintForm> {
    ...
    #[case::both_ends_normalize(
        ...
        Ok(RelationalConstraint::NoncovalentBondEndsSatisfy { bond: NoncovalentBondId(0),
            predicates: [Box::new(AtomConstraintForm::Valence(NumForm::Lit(4))), Box::new(AtomConstraintForm::Degree(NumForm::Lit(2)))] }))]
    #[case::unordered_atom_sets(
        RelationalConstraint::DativeBondDonors { bond: DativeBondId(0), atoms: vec![AtomId(2), AtomId(0), AtomId(2)] },
        Ok(RelationalConstraint::DativeBondDonors { bond: DativeBondId(0), atoms: vec![AtomId(0), AtomId(2)] }))]
    #[case::unordered_endpoints(
        RelationalConstraint::NoncovalentBondEnds { bond: NoncovalentBondId(0), atoms: [AtomId(2), AtomId(0)] },
        Ok(RelationalConstraint::NoncovalentBondEnds { bond: NoncovalentBondId(0), atoms: [AtomId(0), AtomId(2)] }))]
    ...
    fn test_relational_constraint_reframe_by(
        ...
        noncovalent_frame_actions: OverlayFrameActions,
    ) {
        assert_eq!(input.reframe_by(&noncovalent_frame_actions), Some(expected));
    }

    #[rstest]
    fn test_relational_constraint_reframe_by_error() {
        let actions = OverlayFrameActions::from_families(
            DativeBondFrameActions::from_dense(vec![]).expect("actions are admissible"),
            AromaticSystemFrameActions::from_dense(vec![]).expect("actions are admissible"),
            MulticenterBondFrameActions::from_dense(vec![]).expect("actions are admissible"),
            NoncovalentBondFrameActions::from_dense(vec![]).expect("actions are admissible"),
            StereoAtomFrameActions::from_dense(vec![]).expect("actions are admissible"),
            StereoBondFrameActions::from_dense(vec![]).expect("actions are admissible"),
        );
        ...
        assert_eq!(input.reframe_by(&actions), None);
    }
}
