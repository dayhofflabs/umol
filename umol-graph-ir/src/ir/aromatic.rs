...
impl AromaticSystems {
    ...
    pub(crate) fn restore(
        &mut self,
        compaction: &Compaction<AromaticSystemId>,
        ...
        .expect("aromatic system compaction contains valid relation ids");
        let removed = removed
            .into_iter()
    ...
    pub(crate) fn replace_atoms(&mut self, id: AromaticSystemId, atoms: &[AtomId]) {
        let nodes: Vec<NodeId> = atoms.iter().copied().map(NodeId::from).collect();
        Arc::make_mut(&mut self.0).replace_participants(id.into(), &nodes);
    }

    pub(crate) fn replace_atom(&mut self, id: AromaticSystemId, position: usize, atom: AtomId) {
        let position =
    ...
    pub(crate) fn insert_atom(&mut self, id: AromaticSystemId, position: usize, atom: AtomId) {
        let position =
            ParticipantPosition(u32::try_from(position).expect("atom position fits u32"));
        Arc::make_mut(&mut self.0).insert_participant(id.into(), position, atom.into());
    }

    pub(crate) fn remove_atom(&mut self, id: AromaticSystemId, position: usize) {
        let position =
            ParticipantPosition(u32::try_from(position).expect("atom position fits u32"));
...
mod tests {
    ...
    fn test_aromatic_systems_add() {
        let mut systems = AromaticSystems::default();
        let attributes = AromaticSystemForm::from_electrons(vec![1, 2, 3]);
        let id = systems.add(&[AtomId(4), AtomId(1), AtomId(4)], attributes.clone());

        assert_eq!(id, AromaticSystemId(0));
        assert_eq!(
            systems.atoms(id).collect::<Vec<_>>(),
            vec![AtomId(4), AtomId(1), AtomId(4)]
        );
        assert_eq!(systems.attributes(id), &attributes);
        assert_eq!(
            systems.incident_ids(AtomId(4)).collect::<Vec<_>>(),
            vec![id]
        );
    ...
    #[rstest]
    fn test_aromatic_systems_tracked_remove() {
        let original = AromaticSystems::new(vec![
            (vec![AtomId(0)], AromaticSystemForm::from_electrons(vec![1])),
            (vec![AtomId(1)], AromaticSystemForm::from_electrons(vec![2])),
            (vec![AtomId(2)], AromaticSystemForm::from_electrons(vec![3])),
        ]);
        let mut systems = original.clone();
        let compaction = systems.tracked_remove(&[AromaticSystemId(2), AromaticSystemId(0)]);
        let mut plain = original.clone();
        plain.remove(&[AromaticSystemId(0), AromaticSystemId(2)]);

        assert_eq!(
            compaction,
            Compaction::new(3, vec![AromaticSystemId(0), AromaticSystemId(2)]).unwrap()
        );
        assert_eq!(systems, plain);
        assert_eq!(
            systems.atoms(AromaticSystemId(0)).collect::<Vec<_>>(),
            vec![AtomId(1)]
        );
        systems.restore(
            &compaction,
            vec![
                (
                    AromaticSystemId(2),
                    vec![AtomId(2)],
                    original.attributes(AromaticSystemId(2)).clone(),
                ),
                (
                    AromaticSystemId(0),
                    vec![AtomId(0)],
                    original.attributes(AromaticSystemId(0)).clone(),
                ),
            ],
        );
    ...
    #[rstest]
    fn test_aromatic_systems_replace_atoms() {
        let first = AromaticSystemForm::from_electrons(vec![1, 2]);
        let second = AromaticSystemForm::from_electrons(vec![3]);
        let mut systems = AromaticSystems::new(vec![
            (vec![AtomId(1), AtomId(2)], first.clone()),
            (vec![AtomId(3)], second.clone()),
        ]);
        systems.replace_atoms(AromaticSystemId(0), &[AtomId(4), AtomId(4), AtomId(1)]);

        assert_eq!(
            systems.atoms(AromaticSystemId(0)).collect::<Vec<_>>(),
            vec![AtomId(4), AtomId(4), AtomId(1)]
        );
        assert_eq!(systems.attributes(AromaticSystemId(0)), &first);
        assert_eq!(
            systems.atoms(AromaticSystemId(1)).collect::<Vec<_>>(),
            vec![AtomId(3)]
        );
        assert_eq!(systems.attributes(AromaticSystemId(1)), &second);
        assert_eq!(
            systems.incident_ids(AtomId(4)).collect::<Vec<_>>(),
    ...
    #[rstest]
    fn test_aromatic_systems_replace_atom() {
        let mut systems = AromaticSystems::new(vec![(
            vec![AtomId(1), AtomId(2)],
            AromaticSystemForm::default(),
        )]);
        systems.replace_atom(AromaticSystemId(0), 1, AtomId(4));
        assert_eq!(
            systems.atoms(AromaticSystemId(0)).collect::<Vec<_>>(),
            vec![AtomId(1), AtomId(4)]
        );
        assert!(!systems.has_incident(AtomId(2)));
        assert!(systems.has_incident(AtomId(4)));
    }

    #[rstest]
    fn test_aromatic_systems_insert_atom() {
        let mut systems = AromaticSystems::new(vec![(
            vec![AtomId(1), AtomId(2)],
            AromaticSystemForm::default(),
        )]);
        systems.insert_atom(AromaticSystemId(0), 1, AtomId(4));
        systems.insert_atom(AromaticSystemId(0), 3, AtomId(5));
        assert_eq!(
            systems.atoms(AromaticSystemId(0)).collect::<Vec<_>>(),
            vec![AtomId(1), AtomId(4), AtomId(2), AtomId(5)]
        );
        assert_eq!(
            systems.incident_ids(AtomId(5)).collect::<Vec<_>>(),
            vec![AromaticSystemId(0)]
        );
    }

    #[rstest]
    fn test_aromatic_systems_remove_atom() {
        let mut systems = AromaticSystems::new(vec![(
            vec![AtomId(1), AtomId(2)],
            AromaticSystemForm::default(),
        )]);
        systems.remove_atom(AromaticSystemId(0), 0);
        systems.remove_atom(AromaticSystemId(0), 0);
        assert_eq!(
            systems.atoms(AromaticSystemId(0)).collect::<Vec<_>>(),
            Vec::<AtomId>::new()
        );
        assert!(!systems.has_incident(AtomId(1)));
        assert!(!systems.has_incident(AtomId(2)));
    }
    ...
    fn test_aromatic_systems_tracked_compact() {
        let original = AromaticSystems::new(vec![
            ...
            (
                vec![AtomId(1), AtomId(4)],
                AromaticSystemForm::from_electrons(vec![3, 4]),
            ),
            (
                vec![AtomId(3), AtomId(5)],
                AromaticSystemForm::from_electrons(vec![5, 6]),
            ),
        ]);
        ...
        assert_eq!(
            compacted.atoms(AromaticSystemId(1)).collect::<Vec<_>>(),
        ...
        compacted.restore(
            ...
            vec![(
                AromaticSystemId(1),
                vec![AtomId(1), AtomId(4)],
                original.attributes(AromaticSystemId(1)).clone(),
            )],
        );
