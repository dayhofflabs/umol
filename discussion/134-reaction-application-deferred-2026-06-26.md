...
**Done 2026-07-01** — in `umol-ast/tests/property/reaction.rs` (feature `proptest`), on a new `overlay_reaction_strategy` (DAMN-overlay lhs via `from_parts`; `build_reaction` extended to co-remove a deleted atom's incident overlays, keeping the generated reactions DPO-valid). Properties landed and green: `compose_sound_overlay` (P1 soundness), `compose_dangling_free` (P3 dangling, via `DpoValidator`), `compose_rc_anchored_subset` (P2), `apply_reproduces_right_overlay` (probe: plain overlay reaction `apply == right()`), `span_roundtrip` (reaction↔span fidelity), and `compose_well_formed_overlay` (every composite's `apply == right()`). The last was `#[ignore]`d on the first cut (2026-06-30); it surfaced the three apply/remap bugs listed below, and is enabled and green now they are fixed. Coverage completed 2026-07-01 for the cheap properties: `compose_determinism` (P4), `compose_canonical_deltas` (P3), `compose_distinct_overlays` (P6), and deterministic `disjoint_sum` / `disjoint_rc_anchored` cases in `test_reaction_ast_compose` (P5). See the per-property coverage line under **Properties** below. **P1 completeness (`Full`: seq ⊆ composed) remains the one deferred property** — see the enumeration item below for its real root cause.

**Overlap enumeration is maximal-only — the root of P1 incompleteness (graph-core split done 2026-07-01; compose rewire pending).** `compose` enumerates R_A ∩ L_B overlaps via `enumerate_common_subgraphs`, which was Bron–Kerbosch **maximal**-clique enumeration. Bron–Kerbosch returns only *maximal* (non-extendable) common subgraphs, so every non-maximal partial overlap and the empty overlap are dropped whenever any larger overlap exists — yet each distinct overlap is a distinct sequential composite. So `compose(Full)` is systematically **incomplete**, and this — not a VF2/multiplicity subtlety — is why P1 completeness cannot hold. (It is also why the P5 test had to be constructed on disjoint inputs: the empty overlap is enumerated only when it is itself maximal.) These are *different tasks*, not two algorithms for one task, so graph-core now separates them (each its own entry + algorithm enum, mirroring the existing MCS split): `maximal_common_subgraphs` + `MaximalCommonSubgraphAlgorithm::BronKerbosch` (the former behaviour, renamed) and a new `enumerate_common_subgraphs` + `CommonSubgraphEnumerationAlgorithm::Backtracking` that enumerates **every** clique of the modular product (un-pivoted backtracking, each clique once in ascending-vertex order, empty included; shared `modular_product` + `subgraphs_from_cliques` helpers; materialized — a lazy/bounded variant is a later optimization given the exponential worst case). `compose` still calls `maximal_common_subgraphs` for now (behaviour unchanged, all green). **Follow-up (compose rewire):** point `compose` at `enumerate_common_subgraphs`; this enlarges every result set (partial + empty overlaps now appear), so the exact-expected `test_reaction_ast_compose` cases must be reworked (e.g. `fuse` now also yields the disjoint sum), the `disjoint_*` P5 cases fold back into general P1, and P1 completeness (`seq ⊆ composed`) can finally be asserted.
...
- **P1 — apply-equivalence (master, `Full`).** For every host `H`: `⋃_{C ∈ compose(A,B,Full)} C.apply(H)` equals `seq(H)` **as a set** of canonical products. (Soundness: every composite product is a sequential product. Completeness: every sequential product is some composite's product.) Multiset/multiplicity equality is a stronger variant to attempt; if duplicate overlaps or symmetric automorphisms break exact multiplicity, fall back to set equality and assert multiplicity separately. **Covered:** soundness + self-consistency; **completeness deferred.**
