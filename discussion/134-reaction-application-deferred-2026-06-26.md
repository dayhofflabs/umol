...
- **S4b** graph-core (sequence after S4a lands): same unification one layer down — `Graph::induced_subgraph` returns `Correspondence<NodeId>` instead of the bespoke `Embedding<'a>`; `extract` / `host_nodes` become ops over (host, correspondence); retire `Embedding`; migrate `ring.rs`. Graph-core `Correspondence` stays `left`/`right`. `[dep: S4a]`
