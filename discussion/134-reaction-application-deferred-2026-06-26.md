...
**S5 — DSL surface** (additive) **Skipped**
...
- **S6a** graph-core: converges with S4b on `GraphCorrespondence` (node + edge) — every subgraph producer returns it: subiso→`Vec<GraphCorrespondence>` (moves the `edge_mates` step down from `substructure.rs`), MCIS / MCES / maximal / complete→`GraphCorrespondence`. This **subsumes the `Mces { correspondence, edge_count }` wrapper**: each objective is a family `mate_count()` (MCES = `edges().mate_count()`, MCIS = `nodes().mate_count()`), so no separate `edge_count` field. Retire `CommonSubgraph` **and** `Mces`; migrate callers (incl. `substructure.rs` subiso usage). **red→green**. `[dep: S0b, S4b; sequence after S4]`
