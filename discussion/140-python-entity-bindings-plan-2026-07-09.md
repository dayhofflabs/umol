...
- **S2 — constraint container + view (macro, per entity).** `[dep: S1]`
  ...
  - **S2b** macro → `<E>ConstraintsView` + backing with the **own-value arm only** (`<E>(Py<StereoAtomAst>)`; Molecule arm deferred to S4a) + `read`/`with_mut` + mapping methods; add the `View` arm to `<E>ConstraintsUpdate`/`<E>ConstraintsArg` (and make `<E>ConstraintsArg::to_ast` fallible). **Absorbs what S3a deferred:** the value pyclass's `constraints` **getter** (returns the view backed on `slf`), the value's `inner_mut` (consumed by the view's `with_mut`), un-gate the value's `inner`, and flip `set_constraints` to fallible (`?` on the now-fallible `Arg::to_ast`). `[dep: S2a, S3a]`
