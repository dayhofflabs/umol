...
Status: **COMPLETE (2026-07-13) — all seven entity slices B1–B7 bound.** **B1 · Bond slice DONE incl. the view half** (value + WET constraint surface +
...
staged impl plan* for the WET template. **B6/B7 · stereo COMPLETE** (2026-07-13): the overlay
sub-project — `umol-py/src/stereo.rs`, macro-generated over stereo atoms + stereo bonds (S0–S5):
leaf values, the 4-key sub-keyed constraint surface (`stereo_constraints!` container + live view),
`StereoAtomAst`/`StereoBondAst` value pyclasses (`stereo_value!`; the symmetric AST `Display` half
was filled in upstream, mirroring `NoncovalentBondAst`), `StereoAtomView`/`StereoBondView` +
`StereoAtomViews`/`StereoBondViews` (`stereo_view!`/`stereo_views!`, with `coincident`/`connecting`
content lookups), molecule-backed constraint views, `mol.stereo_{atoms,bonds}` accessors, and
`from_parts(stereo_atoms=[], stereo_bonds=[])` kwargs. Also renamed every `from_parts` kwarg and
`__repr__` label to its full accessor name (`dative_bonds`/`aromatic_systems`/`multicenter_bonds`/
`noncovalent_bonds`). **541 Rust unit + 379 pytest green, clippy/ruff/fmt clean.** Deferred beyond
...
- **S1b — DONE** *(breaking→green)* — renamed Python `MoleculeAst.from_atoms_and_bonds` →
  ...
  directly). Added a Rust unit test `test_molecule_from_parts` covering the new `dative` wiring,
...
- **S0a — DONE** *(additive)* — `ElectronCountsAst` pyclass enum `{ Undetermined(), Lit(Vec<i64>) }`
  (`boolean.rs` leaf pattern): `to_ir`, `__eq__`/`__hash__`/`__repr__`, `as_lit() -> Option<list[int]>`.
  New module `umol-py/src/electrons.rs`; registered in `lib.rs` + `__init__.py`; 5 Rust unit tests
  (roundtrip + as_lit); `import umol` + 266 pytest green. `from_ir` gated `#[cfg(test)]` (test-only
...
- **S0b — DONE** *(additive)* — `ElectronCountsArg` (`FromPyObject` enum `{ Lit(Vec<i64>),
  Ast(Py<ElectronCountsAst>) }`) + `to_ir(py)`, mirroring `ValueArg`; a bare `list[int]` coerces to
  `Lit`. Gated `#[cfg(test)]` (first consumer is the S1c value setter — avoids a dead-code warning)
  with a `to_ir` mapping test (both variants); **S1c un-gates it** with `from_ir`. `[dep: S0a]`
...
- **S1a — DONE** *(additive)* — `AromaticSystemConstraintKey { ElectronCount() }` +
  ...
  `key`/`__eq__`/`__hash__`/`__repr__` + `from_ir`/`to_ir`. One key only — no `RingScope`/
  `RingMembership`; the unit key makes `Key::from_ir` infallible (no `py`/`into_py_variant`) and the
  constraint `key()` getter returns the key directly (not `PyResult`). `AromaticSystemConstraintAst::
  from_ir` + the `into_py_variant` import are `#[cfg(test)]`-gated (no non-test consumer until S1b —
...
- **S1b — DONE** *(additive)* — `AromaticSystemConstraintsAst` container: the uniform mapping API
  ...
  **deferred to S1c** (both reference the S1c view type); the container's `inner_mut` **omitted** (its
  ...
  `from_inner`; un-gated `AromaticSystemConstraintAst::from_ir` + `into_py_variant` (iterators consume
...
- **S1c — DONE** *(additive)* — `AromaticSystemAst` value pyclass: `new(electrons: ElectronCountsArg,
  ...
  **Molecule arm deferred to S3** (its only constructor is `AromaticSystemView`). Added (deferred from
  ...
  variant/`apply` arm. **Un-gated `ElectronCountsAst::from_ir` + `ElectronCountsArg` (S0)** — the value
...
- **S2a — DONE** *(additive)* — added `aromatic=Vec::new()` kwarg to `from_parts` (`(atoms, *,
  ...
  migration. Extended `test_molecule_from_parts` to cover the aromatic wiring (asserts via
...
2. **Merge constraint key+enum+container into one stage (S1a)** so `MulticenterBondConstraintAst::from_ir`
...
- **S1a — DONE** *(additive)* — `MulticenterBondConstraintKey { ElectronCount() }` +
  `MulticenterBondConstraintAst { ElectronCount(Py<ValueAst>) }` (unit key ⇒ infallible `Key::from_ir`,
  ...
  cfg-test `from_inner`). `from_ir` **not** gated (container consumes it — the B4 improvement). Verbatim
...
- **S2a — DONE** *(additive)* — added `multicenter=Vec::new()` kwarg to `from_parts` (`(atoms, *, bonds=[],
  ...
  Extended `test_molecule_from_parts` (asserts via `inner().multicenter_bonds()` count + `atom_ids`);
...
- **S0 — kind leaf.**
  - **S0a** `noncovalent.rs`: `NoncovalentBondKind` (fieldless value pyenum, 5 variants, hashable) +
    ...
    `__repr__` + `from_ir`/`to_ir`) + `NoncovalentBondKindArg` (bare-enum | `Ast` passthrough); register
...
- **S1 — constraint half** (independent of S0). Merged into one subitem per the B3/B4 lesson (avoids
  `#[cfg(test)]`-gating `from_ir`; the container's `from_ir` has no non-test consumer until S3's view,
...
- **S3 — view + collection + molecule wiring.**
  ...
  - **S3c** `molecule.rs`, `lib.rs`, `__init__.py`, `tests/test_noncovalent.py`: `mol.noncovalent_bonds`
    ...
    `test_molecule_from_parts`; final registration/exports check. **Additive.** `[dep: S3b]`
...
- **Write the constraint surface in the resolve-before-borrow form from the start** (self-alias panic
  ...
  `value.to_ir(py)?` / `with_mut(|cs| other.apply(...))` form — it panics on `x.constraints =
  x.constraints`. Add the four self-alias regression tests per entity. The molecule-view
  `set_constraints(&self)` single-assignment form (`owner.borrow_mut()…= value.to_ir()?`) is safe as-is
...
- **S0 — stereo leaves (foundation).**
  - **S0a — DONE** *(additive)* — `StereoKind` (6-variant) + `StereoLigandKind` (3) + `Topicity` (3) + `Stereogenicity` (3), fieldless hashable pyenums (`NoncovalentBondKind` pattern), `from_ir`/`to_ir` `#[cfg(test)]`-gated. Registered + exported + roundtrip tests. `[dep: —]`
  - **S0b — DONE** *(additive)* — `StereoLigand` pyclass `{atom_id: u32, kind}` (`#[pyclass(eq, hash, frozen, from_py_object)]`, getters, eval-able `__repr__`, `new`; `from_ir`/`to_ir` gated). `[dep: S0a]`
  - **S0c — DONE** *(additive)* — `StereoConfigurationAst` (`Undetermined() | Kinded(StereoKind, Py<StereoCosetAst>)`, `kind`/`coset` getters, eq/hash/repr via `to_ir`) + `StereoConfigurationArg` (Th/Ct shorthand → `Kinded` + `StereoConfigurationAst` passthrough). Ungated `StereoKind::to_ir` (live). `StereoConfiguration` ground **deferred** (no read/write consumer). `[dep: S0a]`
- **S1 — constraint value leaves.** `[dep: S0]`
  - **S1a — DONE** *(additive)* — helper leaves `Orientation` (Proper/Improper pyenum mirroring `umol_perm::Orientation`, needed by `OrientedLigandPermutation` — not called out in the original list), `LigandPermutation` (wraps the bound `Permutation`, structural eq/hash), `OrientedLigandPermutation` `{permutation, orientation}`, `StereoLigandPair` `{first, second}` u32 (`new` delegates to `AstStereoLigandPair::new` for identical normalization; `from_ir` live), `TopicityRelationAst` (`Undetermined|Lit|LitSet|NotSet` pyclass enum, `ValueAst` pattern, `as_lit`). Added `PartialOrd, Ord` to the pyo3 `Topicity` derive for `BTreeSet<Topicity>`; ungated `Topicity::to_ir` (live consumer `TopicityRelationAst::to_ir`). 75 Rust stereo tests + Python check green, clippy/fmt clean. `[dep: S0b]`
  - **S1b — DONE** *(additive)* — the four constraint values: `StereogenicityAst` (relation-enum, exact `TopicityRelationAst` shape; `Stereogenicity` got `PartialOrd, Ord` + ungated `to_ir`), `LigandSymmetryAst` `{permutation: OrientedLigandPermutation, present: Py<BooleanAst>}`, `FluxionalityAst` `{permutation: LigandPermutation, present: Py<BooleanAst>}`, `TopicityAst` `{pair: StereoLigandPair, relation: Py<TopicityRelationAst>}`. The three structs take `Py<…>` for their pyclass-valued members + `to_ir`-based eq/hash; `present` coerces via the existing `BooleanArg`, `relation` via a new `TopicityRelationArg` (`Topicity | TopicityRelationAst`, `to_py`). Bound the **inherent** `matches` on the three structs (delegates to the AST inherent `matches` — these are *not* `Lattice` in the AST; see the separate Rust-side task to make them semilattices). Ungated the five leaf `to_ir`s (Orientation/LigandPermutation/OrientedLigandPermutation/StereoLigandPair/Stereogenicity) — now live via the struct eq/hash. **Deferred to S2:** `StereogenicityArg` (no consumer until the container setter) and `TopicityRelationArg::to_ir` (only `to_py` is used here). 91 Rust stereo tests + Python check green, clippy/fmt clean. `[dep: S1a]`
- **S2 — constraint container + view (macro, per entity).** `[dep: S1]`
  - **S2a — DONE** *(additive)* — `stereo_constraints!` macro (in `stereo.rs`), instantiated for `StereoAtom` + `StereoBond`, generating per entity: `<E>ConstraintKey` (4 keys — 3 sub-keyed `Py<OrientedLigandPermutation>`/`Py<LigandPermutation>`/`Py<StereoLigandPair>` + bare `Stereogenicity()`) + `<E>ConstraintAst` (4-variant, `key`/eq/hash/repr) + `<E>ConstraintsUpdate`/`Resolved…`/`<E>ConstraintsArg` (resolve-before-borrow; `update` takes `slf: Py<Self>`) + `<E>ConstraintsAst` container (`new`/`repr`/`set`/`pop`/`update`/`get`/`__getitem__`/`__delitem__`/`__contains__`/`__len__` + `__iter__`/`keys`/`values`/`items` + the stereo per-key accessors `ligand_symmetries`/`ligand_symmetry`/`fluxionalities`/`fluxionality`/`topicities`/`topicity`/`stereogenicity` + `inner`/`from_inner`) + 3 return-only iter pyclasses. Un-gated the 10 constraint-consumed `from_ir`s (kept `StereoKind`/`StereoLigandKind`/`StereoLigand`/`StereoConfigurationAst` gated for S3/S4). Macro needs 12 explicit name params (no `paste!`). **Deferred:** the `View` arm on `…Update`/`…Arg` → S2b (with the entity view); `…Arg`/`…Arg::to_ir`/`from_inner` are `#[cfg(test)]`-gated until S3/S2b consume them (currently exercised only by tests). Registered the 6 pyclasses (key/constraint/container × 2) in `lib.rs` + `__init__.py`; iters unregistered (return-only). 11 StereoAtom + 1 StereoBond Rust tests; Python-verified mapping API. 491 lib + 103 stereo tests, clippy/fmt clean. `[dep: S1b]`
  - **S2b — DONE (2026-07-13)** *(additive)* — extended `stereo_constraints!` (+3 params `$value`/`$view`/`$backing`) to generate per entity: `<E>ConstraintsBacking` enum (own-value `Value(Py<$value>)` arm only; `Molecule` arm deferred to S4a) + `<E>ConstraintsView` pyclass `{ backing }` with `read`/`with_mut` (own-value arm reads `entity.inner().constraints` / writes `entity.inner_mut().constraints` in place) + the full mapping API mirroring the container but read-through: `__repr__` (count form), `set`/`pop`/`__delitem__`/`update`/`__len__`/`__iter__`/`keys`/`values`/`items`/`get`/`__getitem__`/`__contains__` + the 7 stereo accessors (`ligand_symmetries`/`ligand_symmetry`/`fluxionalities`/`fluxionality`/`topicities`/`topicity`/`stereogenicity`; all `PyResult` + `py` since they read through a fallible backing, unlike the container's infallible ones). Added the `View(Py<$view>)` arm to `<E>ConstraintsUpdate` (resolve → `Overlay`) and `<E>ConstraintsArg` (and made `<E>ConstraintsArg::to_ir` fallible → `PyResult`). **Absorbed the S3a deferrals** in `stereo_value!` (+2 params `$view`/`$backing`): the value's `constraints` getter (`slf: Py<Self>` → `$view { backing: Value(slf) }`), un-gated `inner`, added `inner_mut`, flipped `set_constraints` to fallible (`?` + `PyResult<()>`, RHS-snapshot-before-borrow). Registered the 2 view pyclasses in `lib.rs` + `__init__.py`. 10 view/getter Rust tests (incl. both self-alias regressions: `x.constraints = x.constraints` via `Arg::View`, and `view.update(own_view)`). 130 stereo tests green, clippy/fmt clean. `[dep: S2a, S3a]`
- **S3 — value pyclass (macro, per entity).** `[dep: S0c, S2]`
  - **S3a — DONE (2026-07-13)** *(additive)* — `stereo_value!` macro (in `stereo.rs`), instantiated for `StereoAtom` + `StereoBond`, generating per entity a `StereoAtomAst`/`StereoBondAst` value pyclass `{configuration, constraints}`: `new(configuration, *, constraints=None)` (via `StereoConfigurationArg` + optional container), `configuration` get/set (via `StereoConfigurationArg`), `set_constraints` (whole-container wipe-and-set, RHS-first snapshot-before-borrow), `asdict` (`configuration` + a `constraints` list of the entries), `inner`/`from_inner` (`#[cfg(test)]`). **`parse`/`__str__`/`__repr__` ARE included** — the earlier "likely no string form" guess was wrong: the entity ASTs have `FromStr` (`"Th0"`/`"Ct0"`), but only the `*Dsl` boundary carried `Display`; **filled the missing half — added `Display for StereoAtomAst`/`StereoBondAst` in `umol-ast/dsl/stereo.rs`, mirroring `Display for NoncovalentBondAst`** (delegates to `<E>Dsl::from_ref(self).fmt`). Un-gated `StereoKind::from_ir` + `StereoConfigurationAst::from_ir` + `StereoConfigurationArg` + the two `<E>ConstraintsArg`s (now consumed non-test). **Deferred to S2b:** the `constraints` **getter** (returns the view — mutually dependent with this pyclass) and `inner_mut` (consumed only by the view); `set_constraints` is infallible while the `Arg` is container-only and goes fallible when the `View` arm lands. Registered the 2 value pyclasses in `lib.rs` + `__init__.py`. 15 StereoAtom + 3 StereoBond Rust tests. Stereo tests green (17 new), clippy/fmt clean on both crates. `[dep: S0c, S2a]` (the S2b dep is inverted — the getter, not the pyclass, needs the view)
- **S4 — view + collection (macro, per entity).** `[dep: S3]`
  - **S4a — DONE (2026-07-13)** *(additive)* — **first** added the `Backing::Molecule { owner: Py<MoleculeAst>, id: $ast_id }` arm to the constraints view in `stereo_constraints!` (+4 params `$ast_id`/`$namespace`/`$entity_mut`/`$id_error`): `read` rebuilds `molecule.inner().stereo_atoms().get(*id)` → `view.ast.constraints` (`IndexError` if gone); `with_mut` writes `owner.borrow_mut().inner_mut().stereo_atom_mut(*id).ast.constraints`. **Then** the new `stereo_view!` macro → `StereoAtomView`/`StereoBondView` `{ owner: Py<MoleculeAst>, id: $ast_id }` with a `view(molecule)` transient-view rebuild helper: read-only `id` (`u32`), `site_id` (`u32`, `.site_id().0`), `ligands` (`list[StereoLigand]`, from `view.ligand_frame()`), `kind` (from configuration), `coset` (from configuration); settable `configuration` (via `StereoConfigurationArg` through `stereo_{atom,bond}_mut`); `constraints` get (→ molecule-backed constraints view) / set (`stereo_{atom,bond}_mut(...).ast.constraints = arg.to_ir()?`); `asdict` (`configuration` + `constraints` list, read through the view). Un-gated `StereoLigand::from_ir` + `StereoLigandKind::from_ir` (consumed by `ligands`) and moved `StereoLigand`/`StereoLigandKind` AST imports out of the `#[cfg(test)]` block. Registered the 2 view pyclasses in `lib.rs` + `__init__.py`. 12 view tests (via `MoleculeParts { stereo_atoms/stereo_bonds }` fixtures): id/site_id/ligands(frame order)/kind/coset/configuration get+set/molecule-backed constraints write-through/set_constraints/asdict/id-out-of-range + a `StereoBondView` confirmation. 142 stereo tests green, clippy/fmt clean. `[dep: S3a, S2b]`
  - **S4b — DONE (2026-07-13)** *(additive)* — new `stereo_views!` macro → `StereoAtomViews`/`StereoBondViews` `{ owner: Py<MoleculeAst> }` + a private `StereoAtomViewIter`/`StereoBondViewIter` (unregistered): `__len__`/`__repr__`/`__getitem__` (isize, negative-aware via the generated `resolve_stereo_{atom,bond}_index` free fn)/`__setitem__` (`*stereo_{atom,bond}_mut(id).ast = value.inner().clone()`, site/ligands unchanged)/`__iter__` + the two content lookups: `coincident(site: u32) -> Option<view>` (→ AST `coincident_id($site_id(site))`) and `connecting(site: u32, ligands: list[StereoLigand]) -> Option<view>` (→ AST `connecting_id($site_id(site), &ligands)`, order-independent). Un-gated `StereoLigand::to_ir` + `StereoLigandKind::to_ir` (the `connecting` arg conversion) and moved `AtomId`/added `BondId` AST imports to non-test (the `$site_id` newtypes). `<E>Views::new` is **deferred to S5a** (its only consumer is the `mol.stereo_{atoms,bonds}` accessor). Registered the 2 collections in `lib.rs` + `__init__.py` (iters unregistered). 11 collection tests: len/getitem(±)/getitem-error/setitem/coincident(hit+miss)/connecting(order-independent hit + partial miss)/iter + a `StereoBondViews` confirmation. 153 stereo tests green, clippy/fmt clean. `[dep: S4a]`
- **S5 — molecule wiring.** `[dep: S4]`
  - **S5a — DONE (2026-07-13)** *(additive; B6/B7 COMPLETE)* — re-added `<E>Views::new` (deferred from S4b) to `stereo_views!`; added `mol.stereo_atoms`/`mol.stereo_bonds` getters (→ `StereoAtomViews::new(slf)` / `StereoBondViews::new(slf)`); extended `from_parts` with `stereo_atoms=[(site, [ligands], StereoAtomAst)]` / `stereo_bonds=[(site, [ligands], StereoBondAst)]` kwargs (site → `AstAtomId`/`AstBondId`, ligands via `StereoLigand::to_ir`); added `stereo_atoms`/`stereo_bonds` to the `__repr__` non-zero-extras loop. `tests/test_stereo.py` +16 end-to-end tests (value new/parse-roundtrip/configuration-setter/constraints-kwarg/asdict, molecule from_parts + `stereo_atoms` accessor + view fields, configuration/constraints write-through, coincident, order-independent connecting, setitem, repr). **Also (user directive):** renamed the `from_parts` kwargs **and** `__repr__` names for the covalent-extra families to their full accessor names — `dative`→`dative_bonds`, `aromatic`→`aromatic_systems`, `multicenter`→`multicenter_bonds`, `noncovalent`→`noncovalent_bonds` (atoms/bonds/stereo_atoms/stereo_bonds already full) — so every `from_parts` kwarg and repr label matches its accessor; updated the 5 affected Python tests. `#[allow(clippy::too_many_arguments)]` on `from_parts` (one arg per entity family). **Verified: 541 Rust unit tests, 379 pytest, clippy/ruff/fmt clean.** `[dep: S4b]`
...
- Transient dead-code warnings on the leaves' `from_ir`/`to_ir`/`BooleanArg` — they feed the surface;
