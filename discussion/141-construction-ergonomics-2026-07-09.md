...
#### Further syntax analysis — typed relation sentences (2026-07-11, discussion only)

The sigil variant is visually economical, but it is not one coherent grammar: it combines binary infix
relations (`~`, `->`), postfix site decoration (`@`, `/`), and prefix n-ary constructors (`A`, `M`).
That is more than a parser inconvenience. It hides which expressions introduce entities, which only
refer to them, whether participant order is significant, and where an overlay's identity and attributes
belong. The familiar-looking sigils are also misleading in a pattern language: SMARTS already uses `~`
for *any bond*, `@` for chirality/ring-bond concepts, and `/` and `\` for directional bond stereo
([SMARTS theory](https://www.daylight.com/dayhtml/doc/theory/theory.smarts.html)).

A stronger alternative is to retain the Cypher-like visual path only for the localized covalent
skeleton, but give every overlay a uniform **noun-led, role-labelled relation sentence**:

```text
aromatic ar1 over {n, c2, c3, c3a, c7a} as "*";
dative db1 from {n1, n2} to fe as "2";
multicenter mc1 over {b1, h, b2} as "2";
noncovalent nc1 between donor and acceptor as "Hbd";

stereo atom sa1 at c ordered [n, o, c2, "#h"] as "Th1";
stereo bond sb1 about alkene ordered [a, b, c, d] as "Db1";
```

Approximate common grammar:

```text
overlay       ::= family [name] role-clause ["as" payload]
role-clause   ::= "over" unordered-participants
                | "from" unordered-participants "to" atom-ref
                | "between" atom-ref "and" atom-ref
                | "at" atom-ref "ordered" ligand-list
                | "about" bond-ref "ordered" ligand-list
```

The family is a one-token statement dispatch; every relation can be named; semantic roles are visible
rather than encoded in punctuation; `{...}` can consistently denote an unordered participant set and
`[...]` an ordered frame; and `as "..."` uniformly carries the existing entity-AST payload. This form
also transfers directly to Python without relying on language-specific operator overloading:

```python
m.dative("db1", donors={n1, n2}, acceptor=fe, spec="2")
m.stereo_atom("sa1", site=c, ligands=[n, o, c2, H], spec="Th1")
```

The maximally regular alternative is a Datalog/term form:

```text
aromatic(ar1, atoms: {a, b, c, d, e, f}, spec: "*");
dative(db1, donors: {n1, n2}, acceptor: fe, spec: "2");
stereo_atom(sa1, site: c, ligands: [n, o, c2, "#h"], spec: "Th1");
```

This mirrors the semantics exactly: a Datalog fact is an n-ary relation application, and typed relation
declarations name each argument's domain ([Soufflé facts](https://souffle-lang.github.io/facts),
[relations](https://souffle-lang.github.io/relations)). The sentence form above is a more readable
surface projection of the same AST, not a different model.

##### What the external precedents contribute

- **Kappa and BioNetGen — sites as resources.** Kappa distinguishes free, bound, bound-to-something,
  and unspecified sites; a bond identifier occurs at its endpoint sites. This is a useful model for
  fragments and patterns even though its compact syntax should not be copied wholesale
  ([Kappa manual](https://tools.kappalanguage.org/docs/KaSim-manual-master/KaSim_manual.htm),
  [BioNetGen tutorial](https://bionetgen.org/tutorial)). An overlay-capable port should name a family
  *and role*, for example `port donor1 at n accepts dative.donor`, `port pi1 at c accepts
  aromatic.member`, or `port axis at db accepts stereo_bond.site`; a generic `overlay` port is too weak.
- **Modelica — simple connection syntax, rich elaboration.** A `connect(a, b)` equation recursively
  matches compatible connector components, merges them into typed connection sets, and connects arrays
  pointwise. The precedent is that a readable statement need not expose the elaborate typed wiring it
  generates ([Modelica connectors and connections](https://specification.modelica.org/master/connectors-and-connections.html)).
  This fits the proposed lattice-meet/port-wiring elaborator.
- **SMARTS/SMIRKS — homoiconicity and sigil pressure.** SMARTS reinterprets molecular terms as patterns,
  adds logic and recursive environments, and SMIRKS adds mapped transforms. It also demonstrates the
  long-term ambiguity caused by repeatedly assigning punctuation positional meanings
  ([SMARTS](https://www.daylight.com/dayhtml/doc/theory/theory.smarts.html),
  [SMIRKS](https://www.daylight.com/dayhtml/doc/theory/theory.smirks.html)). Canonical overlay nouns
  leave substantially more room for future pattern logic.
- **Datalog — assertions, not assembly steps.** Molecule construction is naturally a collection of
  typed facts, while recursive rules provide reachability without changing the fact grammar. This
  reinforces the L2 relation-union model and the separation of stored facts from query operations
  ([Soufflé tutorial](https://souffle-lang.github.io/tutorial)).
- **Hypergraph/wiring-diagram formalisms — semantics, not necessarily typography.** The stored structure
  is best called an attributed typed hypergraph; an open fragment is a hypergraph with a typed boundary;
  and composition is gluing compatible boundary ports. A hypergraph category or wiring-diagram operad
  describes that composition, but does not imply that every generator needs an ASCII pictogram
  ([Catlab wiring diagrams](https://algebraicjulia.github.io/Catlab.jl/v0.3/apis/wiringdiagrams/),
  [wiring diagrams as normal forms](https://arxiv.org/abs/2101.12046),
  [hypergraph categories](https://arxiv.org/abs/1806.08304)). Thus "typed hypergraph" is the direct data
  model and "wiring diagram" the compositional semantics; the terms are complementary, not alternatives.

##### Variable topology — quantify subpaths, not isolated glyphs

Modern Cypher/GQL's most transferable idea is a quantifier on a complete parenthesized path pattern:
`((a)-[:R]->(b)){1,3}`. It supports `*`, `+`, `{n}`, `{m,n}`, `{m,}`, and `{,n}`, and variables declared
inside become group variables. Quantifying a subpath is clearer and more general than putting `*` inside
one bond glyph ([Cypher variable-length paths](https://neo4j.com/docs/cypher-manual/current/patterns/variable-length-paths/),
[formal reference](https://neo4j.com/docs/cypher-manual/current/patterns/reference/variable-length-paths/)).
SPARQL property paths contribute sequence, alternative, inverse, `*`, and `+`, but are principally a
language over binary edge labels ([SPARQL property paths](https://www.w3.org/TR/sparql11-property-paths/)).

A future surface could therefore admit a parenthesized path fragment followed by a quantifier, with an
inline predicate scoped to each iteration. Syntax should wait for these semantic choices:

- Are repeated nodes distinct by default: walk, trail, or node-simple/acyclic path?
- Are names inside the repeat collected into ordered group bindings?
- Does zero repetition identify the two boundary ports?
- Can an overlay consume a group, and does that create one overlay over the union or one per iteration?
- In a reaction, is an edit over a group pointwise, aggregate, or forbidden unless cardinality is fixed?

Ground construction cannot resolve a Kleene star to one molecule. A conservative staging is: fixed
`{n}` in ground construction; bounded/ranged quantifiers in patterns; symbolic quantifiers in fragment
or library generators; and quantified reaction edits only after group-binding/correspondence semantics
exist. This preserves homoiconicity at the syntax-tree level without requiring every AST to be valid in
every interpretation.

##### Consequence for reactions and canonical syntax

Noun-led relations compose directly with the existing reaction verbs:

```text
remove dative db1;
add dative db2 from {o} to fe as "2";
modify stereo atom sa1 as "Th2";
```

`add`/`remove`/`modify` remain orthogonal to relation family, matching the EDN span/operational model.
The recommendation is therefore to keep the implemented flat-keyword grammar as the foundation and, if
this surface is revisited, evolve it toward role-labelled relation sentences. Infix sugar could later be
accepted for unambiguous common binary cases, but the canonical form should remain explicit. Aromatic,
multicenter, and stereo relations are attributed relations with symmetry, participant roles, or ordered
frames; treating them as nouns describes their semantics more faithfully than finding another glyph for
each one.
