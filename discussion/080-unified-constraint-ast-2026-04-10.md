...
3. **Implement `GroundMolecule` newtype** and migrate ground-requiring APIs. ~3 days. *(Done 2026-04-12.)*
   - Plain newtype `GroundMolecule(MoleculeAst)` with `new` / `as_ast` / `into_ir`. No `Arc`, no `OnceLock` view caches — deferred to step 5 (benchmark checkpoint decides whether cached views are needed, and if so whether they live on the molecule or in a separate `MoleculeViews` wrapper).
...
4. **Relational matcher over core relations** (no constraint vec support yet). Enough for basic substructure search. ~2 weeks. *(Done 2026-04-12.)*
   - `matches` methods added to `ElementAst`, `IsotopeAst`, `HydrogenAst`, `AromaticValenceAst` (pattern-against-ground matching) in `umol-shared/src/atom_form.rs`.
...
7. **Migrate aromaticity perception** to discover-then-verify. Graph analysis stays; perception output is constraint emissions. ~1 week. *(Discovery half done 2026-04-15.)*
   ...
   - `find_from_ir` on all three perception models (HueckelRule, HMO, Clar) and `AromaticityModel::aromatic_systems_ast` dispatch.
   - `MoleculeAst::set_aromatic_systems` replaces the `VarRelationSet` wholesale with discovered systems.
   - `AromaticityConfig` (strategy + ring enumeration) added to `Solver`. Resolve order is valence → aromaticity, single pass each (2026-04-16): the valence step resolves all atom predicates including `aromatic_valence`, and aromaticity perception reads those resolved predicates. The earlier valence→aromaticity→re-valence stratification was not load-bearing once `narrow_atom` resolves `aromatic_valence` on the first pass; the `is_in_aromatic_system` branch in `candidates_for` was removed with it. `Solver::resolve` returns `Result<Molecule, ResolutionError>`.
...
- **Isotope resolution is not part of the valence step.** Distinct pass, scope TBD. Currently `ResolverCell::finalize` stopgap-grounds `isotope_mass = Natural` when `Undetermined`; this should move to a proper isotope-resolution phase and the finalize stopgap for `isotope_mass` go away.
- **`ResolverCell::finalize` stopgap-grounds bond `charge` and `spin`.** Same story as isotopes — these should fall out of real resolution passes rather than default-filling in finalize. See the TODO in `solver/propagate.rs::finalize`.
- **Tier-2 invariant restoration on `Molecule`** (doc 86 §"Tier 2: physics invariants"). Per-atom electron count, total charge, total spin coupling, and aromatic-system electron count should be expressed as `MoleculeConstraint::Derived` entries and verified in `Solver::resolve`. `DerivedPred::{TotalCharge, TotalSpin, AromaticElectronCount, ValenceSum}` types exist in `ast/constraint.rs` but the solver does not evaluate them. These checks likely went away with `graph_ir::Molecule`; restoration is mechanical once the `Derived` evaluator lands (step 9).
- **`ReactionRule` / `ReactionRuleAst` split and `Pattern` caches** (doc 86 §"Tier 1 (chemist) type hierarchy"). Mirror of the `Molecule` / `MoleculeAst` split: `ReactionRuleAst` is the algebraic form, `ReactionRule` is the chemist-facing wrapper. `Pattern` gets a lazy ring-view cache like `Molecule`'s `OnceLock<RingSet>`; matcher-side caches (per-atom constraint index, sub-pattern dependency graph, packed pattern adjacency) fold in when step 9/10 land. Scope overlap with doc 80 steps 9–10 — sequence this work there rather than as a standalone item.
