...
Structural refactor and all evaluator work landed in phases P0–P4 of plan `sunny-seeking-fiddle`.

- [x] `AtomConstraint` and `BondConstraint` enums (`ast/constraint.rs`) — all 11 `AtomConstraint` variants; `BondConstraint::RingBond` and `BondConstraint::Aromatic`.
- [x] `AtomPattern` and `BondPattern` containers (`api/pattern.rs`).
- [x] `MoleculeConstraint` reshape — flat enum with `AtomPred` / `BondPred`, cross-feature propagators, `SubPattern`, and `And`/`Or`/`Not` combinators.
- [x] `AtomAst` reduced to six base fields. Five derived fields removed.
- [x] Packed atom/bond DSL lifts constraints; serialization unlifts bare per-atom/per-bond constraints back into sugar.
- [x] Registry accepts `AtomPattern`. `ValenceTheory::AtomTyping` filter uses lifted constraints.
- [x] Solver writes lifted constraints during narrowing.
- [x] `ElectronInvariant` propagator (`ops/propagate.rs`): per-atom electron-conservation check matching doc 52 §10.1.3 exactly.
- [x] Module layout, `Chemistry` container, engines `Resolver` / `Validator` / `Matcher`, theories `ValenceTheory` / `AromaticityTheory`.
- [x] **SMARTS-parity `AtomConstraint` evaluators.** `Valence`, `AromaticValence`, `MulticenterValence`, `DonatedPairs`, `AcceptedPairs`, `Degree`, `Connectivity`, `TotalHCount`, `InRing`, `RingCount`, `RingSize` — all in `ops/evaluate.rs`.
- [x] **`BondConstraint` evaluators.** `RingBond`, `Aromatic` — `ops/evaluate.rs`.
- [x] **Cross-feature propagator evaluators.** `AromaticElectronCount`, `MulticenterElectronCount`, `TotalCharge`, `TotalSpin`, `BondOrderSum`, `Connected` — all in `ops/evaluate.rs`. Tier-2 invariant verification unblocked.
- [x] **`SubPattern` matcher recursion.** `Matcher::find_at(pattern, target, pattern_anchor, target_anchor)` backed by `subgraph_isomorphisms_at` in `umol-graph-core::algorithms::subiso`. `SubPattern` AST carries explicit `target_anchor` and `pattern_anchor: AtomIdx` (no implicit atom-0). Empty patterns rejected at call site.
- [x] **Combinator evaluation.** `And` / `Or` / `Not` dispatched in both `MoleculeConstraint::evaluate` and `Matcher::evaluate_remapped`.
- [x] **EDN DSL keys for all constraints.** `:atom-pred`, `:bond-pred`, `:total-charge`, `:total-spin`, `:aromatic-electron-count`, `:multicenter-electron-count`, `:bond-order-sum`, `:connected`, `:sub-pattern` (with `:target-anchor` / `:pattern-anchor` / `:pattern`), `:and`, `:or`, `:not` — all round-trip.
- [x] 617 conformance tests + 3716 lib tests pass.

## Deferred (not blocking)

- **Derived view caches on `Molecule`.** `OnceLock<RingSet>` present; distance matrix, biconnected components, per-atom constraint index, packed pattern adjacency land when the first consumer arrives — "gated on actually needing them".
- **`Chemistry` naming.** Provisional — may rename if usage becomes awkward.
- **Multi-type API surface.** `Molecule`/`MoleculeAst`, `AtomAst`/`AtomPattern`, `BondAst`/`BondPattern` overlap flagged for a dedicated review pass (separate from constraint taxonomy).
- **`:aromatic` keyword alias in `builtin_bond_aliases`.** Requires unifying registry type from `BiMap<String, BondAst>` to `BiMap<String, BondPattern>`. Bundled with the multi-type review.
