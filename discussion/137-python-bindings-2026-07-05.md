...
- **Core type shapes** (the fact that drives the binding design):
  ...
  | --- | --- | --- |
  | Owned, non-generic | `MoleculeAst`, `ReactionSpanAst`, `ReactionDerivation` | Yes, directly |
  ...
  `ReactionDerivation` belongs to the owned row and wraps directly.
...
- **Reaction derivations**: `ReactionDerivation` is a fully owned value and wraps
  directly. Its `lhs`, `rhs`, and correspondence remain valid independently of
  the molecule passed to `apply`; no parallel result representation is needed.
...
> - **2 (by-id accessor 4-path redundancy) — DONE** (2026-07-13). Resolved by removing **all 16 `Index` impls** (8 on `MoleculeAst`, 8 on the entity `*Views`). The by-id read surface is now the idiomatic pair — `mol.atom(id)` (panicking view) + `mol.atoms().get(id)` (checked view) — with raw `&XAst` via `.atom(id).ast`. Rationale: `Index::Output` must be a borrow, so `Index` could *only* return `&XAst`, never a view — it was the forced source of the return-type asymmetry, and everything it offered was already reachable through `.ast`; it also had Rust's `[id]` mean "data" while Python's `atoms[id]` means "view." (The earlier "near-dead" read was wrong — `MoleculeAst` `Index` was used ~25× in `dsl/reaction.rs`.) Migrated ~69 call sites (`reaction.rs` 24, `molecule/tests.rs` 34, `molecule.rs` 1, umol-graph 4, `tests/property/edit.rs` 5) from `X[id]` → `X.<entity>(id).ast`, deleted the 8 `*_views_index` unit tests. **Verified `--all-features --tests`: umol-ast 4568 + property 115, umol-graph 314, clippy (all-features) / fmt clean, workspace builds.** (The property-test sites were behind the `proptest` feature gate — plain `--tests` skipped them; caught only on the full-breadth build.)
