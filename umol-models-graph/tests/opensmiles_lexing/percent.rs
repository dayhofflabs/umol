...
fn percent_leading_zero_tokenized() {
    ...
    let got = Token::lexer(b"%01").map(|t| t.ok()).collect::<Option<Vec<_>>>().unwrap();
    assert_eq!(got, vec![Token::Percent(1)]);
}

#[rstest]
fn percent_zero_tokenized() {
    let got = Token::lexer(b"%00").map(|t| t.ok()).collect::<Option<Vec<_>>>().unwrap();
