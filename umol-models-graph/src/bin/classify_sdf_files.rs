...
fn collect_sdf_files(dir: &Path) -> Result<Vec<std::path::PathBuf>, Box<dyn std::error::Error>> {
    ...
    for entry in fs::read_dir(dir)? {
        ...
        } else if path.extension() == Some(std::ffi::OsStr::new("sdf")) {
            files.push(path);
        }
    }
    Ok(files)
