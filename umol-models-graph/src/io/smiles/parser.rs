...
fn parse_bracket(inner: &[u8], pos_offset: usize)
...
fn parse_core(input: &[u8], flags: SmilesParseFlags) -> Result<Molecule, ParseError> {
    ...
    let mut pstack: SmallVec<[Frame; BRANCH_STACK_DEPTH]> = SmallVec::new(); let mut ring_table: [Option<OpenRing>; RING_TABLE_LEN] = [None; RING_TABLE_LEN];
    ...
    while i < n {
        ...
        if b0 == b'(' { if let Some((_, _, pos)) = pending_bond { return Err(ParseError::TrailingBond { pos }); } if just_closed_group { last_atom_idx=None; pstack.push(Frame::Group{had_atom:false,open_pos:i}); just_closed_group=false; } else { match last_atom_idx { Some(idx)=>pstack.push(Frame::Branch{base:idx,had_atom:false,open_pos:i}), None=>pstack.push(Frame::Group{had_atom:false,open_pos:i}), } } i+=1; continue; }
        if b0 == b')' { if let Some((_, _, pos)) = pending_bond { return Err(ParseError::TrailingBond { pos }); } let Some(frame)=pstack.pop() else { return Err(ParseError::UnbalancedBranchClose { pos: i }); }; match frame { Frame::Branch { base, had_atom, .. } => { if !had_atom { return Err(ParseError::EmptyBranch { pos: i }); } last_atom_idx=Some(base); } Frame::Group { had_atom, open_pos, .. } => { if !had_atom { if i + 1 != n { return Err(ParseError::EmptyGroup { pos: i }); } if i>0 && input[i-1]==b'.' { return Err(ParseError::LeadingDot { pos: i }); } if open_pos != 0 { return Err(ParseError::EmptyGroup { pos: i }); } last_atom_idx=None; just_closed_group=false; } else { just_closed_group=true; if pstack.is_empty() && i + 1 != n { let next = input[i + 1]; if next != b'.' { return Err(ParseError::TopLevelGroupTrailing { pos: i }); } } } } } i+=1; continue; }
        if b0 == b'.' { if let Some((_, _, pos)) = pending_bond { return Err(ParseError::TrailingBond { pos }); } if i==0 { return Err(ParseError::LeadingDot { pos: i }); } if let Some(Frame::Group { had_atom: false, .. }) = pstack.last() { return Err(ParseError::GroupLeadingDot { pos: i }); } if i + 1 == n { return Err(ParseError::TrailingDot { pos: i }); } if input[i + 1] == b'.' { return Err(ParseError::ConsecutiveDot { pos: i }); } last_atom_idx=None; last_aromatic=false; i+=1; continue; }
        if is_digit(b0) { if last_atom_idx.is_none() { return Err(ParseError::LeadingRing { pos: i }); } if let Some(Frame::Branch { had_atom: false, .. } | Frame::Group { had_atom: false, .. }) = pstack.last() { return Err(ParseError::LeadingRing { pos: 0 }); } let idx: usize = (b0 - b'0') as usize; if let Some(seq) = _ring_sequence.as_mut() { let d = idx as u32; if _first_ring_digit.is_none() { _first_ring_digit = Some(d); } seq.push((d, i)); } let bond = pending_bond.take(); let (order_opt, dir_opt) = bond.map_or((None,None), |(o,d,_)|(Some(o),d)); let entry=&mut ring_table[idx]; match entry.take() { None=>{ *entry=Some(OpenRing{ atom_id:last_atom_idx.unwrap(), order:order_opt, dir:dir_opt, open_pos:i, open_aromatic:last_aromatic }); } Some(open)=>{ let _b=last_atom_idx.unwrap(); if let (Some(d1),Some(d2))=(open.dir,dir_opt) { if d1!=d2 { return Err(ParseError::RingBondDirConflict{pos:i, open_pos: open.open_pos}); } } if let (Some(o1),Some(o2))=(open.order,order_opt) { if o1!=o2 { return Err(ParseError::RingBondOrderConflict{pos:i, open_pos: open.open_pos}); } } if open.dir.is_some() || dir_opt.is_some() { let ord = open.order.or(order_opt).unwrap_or(BondOrder::Single); if ord != BondOrder::Single { return Err(ParseError::RingBondOrderConflict{pos:i, open_pos: open.open_pos}); } }
            ...
            let final_dir = open.dir.or(dir_opt); let a=open.atom_id; let b=last_atom_idx.unwrap(); if final_order==BondOrder::Single && open.open_aromatic && last_aromatic { final_order=BondOrder::Aromatic; } builder.on_bond(a,b, BondData{order:final_order,dir:final_dir}); } } i+=1; continue; }
        if b0 == b'%' { if i + 2 >= n || !is_digit(input[i+1]) || !is_digit(input[i+2]) { return Err(ParseError::RingIndexInvalid { pos: i }); } if last_atom_idx.is_none() { return Err(ParseError::LeadingRing { pos: i }); } if let Some(Frame::Branch { had_atom: false, .. } | Frame::Group { had_atom: false, .. }) = pstack.last() { return Err(ParseError::LeadingRing { pos: 0 }); } if let Some(seq) = _ring_sequence.as_mut() { if input[i+1] == b'0' && (input[i+2] >= b'1' && input[i+2] <= b'9') { _percent_padded = true; } let d = ((input[i+1]-b'0') as u32)*10 + (input[i+2]-b'0') as u32; if _first_ring_digit.is_none() { _first_ring_digit = Some(d); } seq.push((d, i)); }
            ...
            let mut final_order = match (open.order, order_opt) { (Some(o1), Some(o2)) => { if o1==o2 { o1 } else { o2 } }, (Some(o), None) | (None, Some(o)) => o, (None, None) => BondOrder::Single };
            let final_dir = open.dir.or(dir_opt); let a=open.atom_id; let b_idx=last_atom_idx.unwrap(); if final_order==BondOrder::Single && open.open_aromatic && last_aromatic { final_order=BondOrder::Aromatic; } builder.on_bond(a,b_idx, BondData{order:final_order,dir:final_dir}); } } i+=3; continue; }
        if matches!(b0, b'-' | b'=' | b'#' | b'$' | b':' | b'/' | b'\\') { if pending_bond.is_some() { return Err(ParseError::ConsecutiveBond { pos: i }); } if last_atom_idx.is_none() { if let Some(Frame::Group { had_atom: false, .. }) = pstack.last() { return Err(ParseError::GroupLeadingBond { pos: i }); } return Err(ParseError::LeadingBond { pos: i }); } let (order,dir)=map_bond(b0); pending_bond=Some((order,dir,i)); i+=1; continue; }
        if b0 == b'[' { let start=i+1; let mut j=start; while j < n && input[j] != b']' { j+=1; } if j >= n { return Err(ParseError::UnbalancedOpenBracket { pos: i }); } let inner=&input[start..j]; let (elem_opt, aromatic, iso_opt, charge_opt, class_opt, h_opt, chir_opt, unknown) = parse_bracket(inner, i)?; let (element, aromatic) = match elem_opt { Some(e) => (e, aromatic), None => (Element::C, false) }; let atom = AtomData { element, isotope: iso_opt, charge: charge_opt, hydrogen_count: h_opt.map(|h| (h as u8).min(u8::MAX)), class: class_opt, aromatic, implicit_h: false, chirality: chir_opt, unknown_symbol: unknown, };
            let curr = builder.on_atom(atom); let old_last = last_atom_idx; if let Some(last)=old_last { if let Some((order,dir,_))=pending_bond.take() { builder.on_bond(last,curr,BondData{order,dir}); } else { if last_aromatic && aromatic { builder.on_bond(last,curr,BondData{order:BondOrder::Aromatic, dir:None}); } else { builder.on_bond_single_fast(last,curr); } } } last_atom_idx = Some(curr); last_aromatic = aromatic; if let Some(top)=pstack.last_mut() { match top { Frame::Branch { had_atom, .. } | Frame::Group { had_atom, .. } => { *had_atom = true } } } i = j + 1; continue; }
        if b0 == b'C' { if i + 1 < n && input[i+1] == b'l' { let curr = builder.on_atom_fast(Element::Cl, true, false); let old_last = last_atom_idx; if let Some(last)=old_last { if let Some((order,dir,_))=pending_bond.take() { builder.on_bond(last,curr,BondData{order,dir}); } else { builder.on_bond_single_fast(last,curr); } } last_atom_idx=Some(curr); last_aromatic=false; if let Some(top)=pstack.last_mut() { match top { Frame::Branch { had_atom, .. } | Frame::Group { had_atom, .. } => { *had_atom = true } } } i += 2; continue; } let curr = builder.on_atom_fast(Element::C, true, false); let old_last = last_atom_idx; if let Some(last)=old_last { if let Some((order,dir,_))=pending_bond.take() { builder.on_bond(last,curr,BondData{order,dir}); } else { builder.on_bond_single_fast(last,curr); } } last_atom_idx=Some(curr); last_aromatic=false; if let Some(top)=pstack.last_mut() { match top { Frame::Branch { had_atom, .. } | Frame::Group { had_atom, .. } => { *had_atom = true } } } i += 1; continue; }
        if b0 == b'B' { if i + 1 < n && input[i+1] == b'r' { let curr = builder.on_atom_fast(Element::Br, true, false); let old_last = last_atom_idx; if let Some(last)=old_last { if let Some((order,dir,_))=pending_bond.take() { builder.on_bond(last,curr,BondData{order,dir}); } else { builder.on_bond_single_fast(last,curr); } } last_atom_idx=Some(curr); last_aromatic=false; if let Some(top)=pstack.last_mut() { match top { Frame::Branch { had_atom, .. } | Frame::Group { had_atom, .. } => { *had_atom = true } } } i += 2; continue; } let curr = builder.on_atom_fast(Element::B, true, false); let old_last = last_atom_idx; if let Some(last)=old_last { if let Some((order,dir,_))=pending_bond.take() { builder.on_bond(last,curr,BondData{order,dir}); } else { builder.on_bond_single_fast(last,curr); } } last_atom_idx=Some(curr); last_aromatic=false; if let Some(top)=pstack.last_mut() { match top { Frame::Branch { had_atom, .. } | Frame::Group { had_atom, .. } => { *had_atom = true } } } i += 1; continue; }
        ...
        if let Some(element) = elem { let curr = builder.on_atom_fast(element, true, false); let old_last = last_atom_idx; if let Some(last)=old_last { if let Some((order,dir,_))=pending_bond.take() { builder.on_bond(last,curr,BondData{order,dir}); } else { builder.on_bond_single_fast(last,curr); } } last_atom_idx=Some(curr); last_aromatic=false; if let Some(top)=pstack.last_mut() { match top { Frame::Branch { had_atom, .. } | Frame::Group { had_atom, .. } => { *had_atom = true } } } i += 1; continue; }
        if matches!(b0, b'b' | b'c' | b'n' | b'o' | b'p' | b's') { let element = match b0 { b'b' => Element::B, b'c' => Element::C, b'n' => Element::N, b'o' => Element::O, b'p' => Element::P, _ => Element::S }; let curr = builder.on_atom_fast(element, true, true); let old_last = last_atom_idx; if let Some(last)=old_last { if let Some((order,dir,_))=pending_bond.take() { builder.on_bond(last,curr,BondData{order,dir}); } else { if last_aromatic { builder.on_bond(last,curr,BondData{order:BondOrder::Aromatic, dir:None}); } else { builder.on_bond_single_fast(last,curr); } } } last_atom_idx=Some(curr); last_aromatic=true; if let Some(top)=pstack.last_mut() { match top { Frame::Branch { had_atom, .. } | Frame::Group { had_atom, .. } => { *had_atom = true } } } i += 1; continue; }
        // Bare invalid element token
        if b0.is_ascii_alphabetic() { return Err(ParseError::InvalidElement { pos: i }); }
        if b0 == b'*' { let atom = AtomData { element: Element::C, isotope: Some(0), charge: Some(0), hydrogen_count: Some(0), class: None, aromatic: false, implicit_h: false, chirality: None, unknown_symbol: true }; let curr = builder.on_atom(atom); let old_last = last_atom_idx; if let Some(last)=old_last { if let Some((order,dir,_))=pending_bond.take() { builder.on_bond(last,curr,BondData{order,dir}); } else { builder.on_bond_single_fast(last,curr); } } last_atom_idx=Some(curr); last_aromatic=false; if let Some(top)=pstack.last_mut() { match top { Frame::Branch { had_atom, .. } | Frame::Group { had_atom, .. } => { *had_atom = true } } } i += 1; continue; }
