...
fn build_pi_graph(mol: &Molecule) -> PiGraph {
        ...
        if let BondSymbol::Bond(ord) = b.symbol {
            match ord {
                BondOrder::Double | BondOrder::Aromatic => {
                    let ai = a as usize;
                    let ci = c as usize;
                    if ai < n_atoms { is_pi_center[ai] = true; }
                    if ci < n_atoms { is_pi_center[ci] = true; }
                }
                _ => {}
            }
        }
    ...
    for b in &mol.bonds {
        let (Some(a), Some(c)) = (b.start_atom, b.end_atom) else { continue; };
        let ai = a as usize; let ci = c as usize;
        if ai >= n_atoms || ci >= n_atoms { continue; }
        let (Some(pa), Some(pc)) = (atom_to_pi[ai], atom_to_pi[ci]) else { continue; };
...
pub fn check_aromaticity(
    ...
    if !cfg.enabled {
        return AromaticityArtifacts { checked_atoms: 0, pi_centers: 0, pi_edges: 0, method_used: cfg.method };
    }
    let n_atoms = mol.atoms.len();
    let pg = build_pi_graph(mol);

    let has_aromatic_atoms = mol.atoms.iter().any(|a| a.aromatic == Some(true));
    let has_aromatic_bonds = mol.bonds.iter().any(|b| matches!(b.symbol, BondSymbol::Bond(BondOrder::Aromatic)));
    ...
    if matches!(cfg.method, AromaticityMethod::Hmo | AromaticityMethod::Combined) {
        ...
        for (ai, opt_pi) in pg.atom_to_pi.iter().enumerate() {
            if let Some(pi) = opt_pi { if *pi < pi_to_atom.len() { pi_to_atom[*pi] = ai; } }
        }
        let nearest_int = |x: f32| -> f32 { let r = x.round(); if r < 0.0 { 0.0 } else if r > 2.0 { 2.0 } else { r } };
        ...
        for (bi, b) in mol.bonds.iter().enumerate() {
            if let (Some(a), Some(c)) = (b.start_atom, b.end_atom) {
                ...
                if let BondSymbol::Bond(ord) = b.symbol { bond_lookup.insert(key, (bi, ord)); }
            }
        }
        for &(u, v) in &pg.edges {
            let key = if u < v { (u, v) } else { (v, u) };
            if let Some(&b) = bmap.get(&key) {
                let ai = pi_to_atom[u]; let ci = pi_to_atom[v];
                let mkey = if ai < ci { (ai, ci) } else { (ci, ai) };
                if let Some((_bi, _ord)) = bond_lookup.get(&mkey) {
                    let diff = (b - nearest_int(b)).abs();
                    if diff >= cfg.fractional_bond_threshold { significant_count += 1; }
                }
            }
        }
    ...
    if has_aromatic_tokens && (!has_significant || pg.size == 0) {
        report.push(crate::diagnostics::Diagnostic {
            ...
            details: Some(format!("pi_centers={}, significant_edges={}", pg.size, significant_count)),
        });
    }
    ...
    AromaticityArtifacts { checked_atoms: n_atoms, pi_centers: pg.size, pi_edges: pg.edges.len(), method_used: cfg.method }
}
