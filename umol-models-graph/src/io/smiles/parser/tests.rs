...
use crate::io::smiles::parser::{chain, grammar, tree};
...
fn test_chain_atom(mut parse_state: ParseState, #[case] input: &str, #[case] expected: Atom) {
    let lexer = Lexer::new(input);
    let parser = chain::AtomParser::new();
...
fn test_chain_bond(mut parse_state: ParseState, #[case] input: &str, #[case] expected: Bond) {
    let lexer = Lexer::new(input);
...
#[rstest]
#[case("C", {let mut atom = Atom::from_aliphatic_atom(Element::C); atom.index = Some(0); atom})]
fn test_tree_atom(mut parse_state: ParseState, #[case] input: &str, #[case] expected: Atom) {
    let lexer = Lexer::new(input);
    let parser = tree::AtomParser::new();
    let result = parser.parse(&mut parse_state, lexer);
    assert!(result.is_ok(), "{} should have succeeded", input);
    let result = result.unwrap();
    assert_eq!(result, expected);
...
#[rstest]
#[case("=", Bond::from_order(BondOrder::Double))]
fn test_tree_bond(mut parse_state: ParseState, #[case] input: &str, #[case] expected: Bond) {
    let lexer = Lexer::new(input);
    parse_state.current_atom = 0;
    parse_state.next_atom = 1;
    let parser = tree::BondParser::new();
    let result = parser.parse(&mut parse_state, lexer);
    assert!(result.is_ok(), "{} should have succeeded", input);
    let result = result.unwrap();
    assert_eq!(result, expected);
...
#[case("CC", 2, 1)]
#[case("C=C", 2, 1)]
fn test_tree_chain(mut parse_state: ParseState, #[case] input: &str, #[case] atoms: usize, #[case] bonds: usize) {
    let lexer = Lexer::new(input);
    let parser = tree::ChainParser::new();
    let result = parser.parse(&mut parse_state, lexer);
    assert!(result.is_ok(), "{} should have succeeded", input);
    assert_eq!(parse_state.next_atom, atoms);
    assert_eq!(parse_state.next_bond, bonds);
...
fn test_tree_invalid(mut parse_state: ParseState, #[case] input: &str, #[case] desc: &str) {
    let lexer = Lexer::new(input);
    let parser = tree::TreeParser::new();
    let result = parser.parse(&mut parse_state, lexer);
    assert!(result.is_err(), "{} should have failed", desc);
