...
pub(super) fn sgroup_type<'inp>(
) -> impl Parser<&'inp [u8], Output = SGroupType, Error = NomError<&'inp [u8]>> {
...
pub(super) fn sgroup_subtype<'inp>(
) -> impl Parser<&'inp [u8], Output = SGroupSubtype, Error = NomError<&'inp [u8]>> {
    map_res(take(3usize), move |s: &[u8]| match s {
...
pub(super) fn sgroup_connectivity<'inp>(
) -> impl Parser<&'inp [u8], Output = SGroupConnectivity, Error = NomError<&'inp [u8]>> {
    move |input: &'inp [u8]| {
        fixed_width_partial(
            3usize,
...
pub(super) fn sgroup_multiplier<'inp>(
) -> impl Parser<&'inp [u8], Output = SGroupMultiplier, Error = NomError<&'inp [u8]>> {
    alt((
        map(
            separated_pair(
...
fn sgroup_multiplier_integer<'inp>(
) -> impl Parser<&'inp [u8], Output = SGroupMultiplierTerm, Error = NomError<&'inp [u8]>> {
    map(nom_u32, SGroupMultiplierTerm::Integer)
}

/// Parse a single-character variable multiplier
fn sgroup_multiplier_variable<'inp>(
) -> impl Parser<&'inp [u8], Output = SGroupMultiplierTerm, Error = NomError<&'inp [u8]>> {
...
fn sgroup_multiplier_term<'inp>(
) -> impl Parser<&'inp [u8], Output = SGroupMultiplierTerm, Error = NomError<&'inp [u8]>> {
    alt((sgroup_multiplier_integer(), sgroup_multiplier_variable()))
}

/// Parse arithmetic operator
fn sgroup_multiplier_op<'inp>(
) -> impl Parser<&'inp [u8], Output = SGroupMultiplierOp, Error = NomError<&'inp [u8]>> {
    alt((
...
pub(super) fn sgroup_subscript<'inp>(
...
pub(super) fn sgroup_data_type<'inp>(
) -> impl Parser<&'inp [u8], Output = SGroupDataType, Error = NomError<&'inp [u8]>> {
...
pub(super) fn sgroup_data_display_type<'inp>(
) -> impl Parser<&'inp [u8], Output = SGroupDataDisplayType, Error = NomError<&'inp [u8]>> {
...
pub(super) fn sgroup_data_display_placement<'inp>(
) -> impl Parser<&'inp [u8], Output = SGroupDataDisplayPlacement, Error = NomError<&'inp [u8]>> {
    map_res(take(1usize), move |s: &[u8]| match s {
...
pub(super) fn sgroup_data_display_units<'inp>(
) -> impl Parser<&'inp [u8], Output = SGroupDataDisplayUnits, Error = NomError<&'inp [u8]>> {
    map_res(take(1usize), move |s: &[u8]| match s {
...
pub(super) fn sgroup_data_display_chars<'inp>(
) -> impl Parser<&'inp [u8], Output = SGroupDataDisplayChars, Error = NomError<&'inp [u8]>> {
    map_parser(take(3usize), move |s: &'inp [u8]| {
        let s = s.trim_ascii();
        alt((
