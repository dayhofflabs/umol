...
impl MoleculeLike {
    ...
    /// Add atom to the molecule and update index mappings
    pub fn add_atom(&mut self, atom: AtomLike) -> usize {
        self.graph.add_node(atom).index()
    }

    /// Add bond between two atoms specified by external/MOL indices
    pub fn add_bond(&mut self, idx1: usize, idx2: usize, bond: BondLike) -> usize {
        self.graph
            .add_edge(AtomIndex::new(idx1), AtomIndex::new(idx2), bond)
            .index()
    }
    ...
    /// Get iterator over atoms
    pub fn atoms(&self) -> impl Iterator<Item = &AtomLike> + '_ {
        self.graph.node_weights()
    }

    /// Get immutable reference to atom by index
    pub fn atom(&self, idx: usize) -> Option<&AtomLike> {
        self.graph.node_weight(AtomIndex::new(idx))
    }

    /// Get mutable reference to atom by index
    pub fn atom_mut(&mut self, idx: usize) -> Option<&mut AtomLike> {
        self.graph.node_weight_mut(AtomIndex::new(idx))
    }
    ...
    /// Get iterator over bonds
    pub fn bonds(&self) -> impl Iterator<Item = &BondLike> + '_ {
        self.graph.edge_weights()
    }

    /// Get immutable reference to bond by index
    pub fn bond(&self, idx: usize) -> Option<&BondLike> {
        self.graph.edge_weight(BondIndex::new(idx))
    }

    /// Get mutable reference to bond by index
    pub fn bond_mut(&mut self, idx: usize) -> Option<&mut BondLike> {
        self.graph.edge_weight_mut(BondIndex::new(idx))
    }
...
mod tests {
    ...
    fn test_molecule_sum_formula_glycine() {
        ...
        molecule.add_atom(Atom::new(e!(N))); // 0: N
        molecule.add_atom(Atom::new(e!(H))); // 1: H
        molecule.add_atom(Atom::new(e!(H))); // 2: H
        molecule.add_atom(Atom::new(e!(C))); // 3: C
        molecule.add_atom(Atom::new(e!(H))); // 4: H
        molecule.add_atom(Atom::new(e!(H))); // 5: H
        molecule.add_atom(Atom::new(e!(C))); // 6: C
        molecule.add_atom(Atom::new(e!(O))); // 7: O
        molecule.add_atom(Atom::new(e!(O))); // 8: O
        molecule.add_atom(Atom::new(e!(H))); // 9: H
    ...
    #[test]
    fn test_moleculelike_serialize() {
        let graph = StableGraph::<AtomLike, BondLike, Undirected, usize>::with_capacity(0, 0);
        let sgroups = BTreeMap::new();
        let properties = HashMap::new();
        let molecule = MoleculeLike {
            graph,
            sgroups,
            properties,
        };
    ...
    fn test_moleculelike_sum_formula() {
        ...
        molecule.add_atom(AtomLike::new(AtomSymbol::Element(e!(H))));
        molecule.add_atom(AtomLike::new(AtomSymbol::Element(e!(H))));
        molecule.add_bond(0, 1, BondLike::new(BondType::Single));
        molecule.add_bond(0, 2, BondLike::new(BondType::Single));
    ...
    fn test_moleculelike_sum_formula_glycine() {
        ...
        // Glycine: NH2-CH2-COOH (C2H5NO2)
        let mut molecule = MoleculeLike::new();
        molecule.add_atom(AtomLike::new(AtomSymbol::Element(e!(N)))); // 0: N
        molecule.add_atom(AtomLike::new(AtomSymbol::Element(e!(H)))); // 1: H
        molecule.add_atom(AtomLike::new(AtomSymbol::Element(e!(H)))); // 2: H
        molecule.add_atom(AtomLike::new(AtomSymbol::Element(e!(C)))); // 3: C
        molecule.add_atom(AtomLike::new(AtomSymbol::Element(e!(H)))); // 4: H
        molecule.add_atom(AtomLike::new(AtomSymbol::Element(e!(H)))); // 5: H
        molecule.add_atom(AtomLike::new(AtomSymbol::Element(e!(C)))); // 6: C
        molecule.add_atom(AtomLike::new(AtomSymbol::Element(e!(O)))); // 7: O
        molecule.add_atom(AtomLike::new(AtomSymbol::Element(e!(O)))); // 8: O
        molecule.add_atom(AtomLike::new(AtomSymbol::Element(e!(H)))); // 9: H

        // Bonds
        molecule.add_bond(0, 1, BondLike::new(BondType::Single)); // N-H
        molecule.add_bond(0, 2, BondLike::new(BondType::Single)); // N-H
        molecule.add_bond(0, 3, BondLike::new(BondType::Single)); // N-C
        molecule.add_bond(3, 4, BondLike::new(BondType::Single)); // C-H
        molecule.add_bond(3, 5, BondLike::new(BondType::Single)); // C-H
        molecule.add_bond(3, 6, BondLike::new(BondType::Single)); // C-C
        molecule.add_bond(6, 7, BondLike::new(BondType::Double)); // C=O
        molecule.add_bond(6, 8, BondLike::new(BondType::Single)); // C-O
        molecule.add_bond(8, 9, BondLike::new(BondType::Single)); // O-H
