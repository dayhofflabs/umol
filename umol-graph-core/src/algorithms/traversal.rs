...
/// Algorithm for distance-limited graph neighborhoods.
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub enum NeighborhoodAlgorithm {
    Bfs,
}
...
impl Graph {
    ...
    /// # Semantic properties
    ///
    /// For a source in this graph, the result is exactly the Discover events
    /// from [`Graph::visit_breadth_first`] with that source and depth limit,
    /// projected to `(node, depth)`. Distances are shortest-path distances;
    /// equal-distance nodes retain their BFS discovery order. Disconnected
    /// nodes are excluded, and a zero limit includes only the source.
    pub fn neighborhood(
        &self,
        source: NodeId,
        max_depth: usize,
        ...
        match algorithm {
            NeighborhoodAlgorithm::Bfs => {
                let mut reached = Vec::new();
                let _: ControlFlow<()> =
                    self.visit_breadth_first([source], Some(max_depth), |event| {
                        if let BreadthFirstEvent::Discover { node, depth, .. } = event {
...
mod tests {
    ...
    #[case::path_from_middle(
        ...
        vec![(NodeId(2), 0), (NodeId(1), 1), (NodeId(3), 1), (NodeId(0), 2)]
    )]
    #[case::cycle(
        Graph::new(4, &[[0, 1], [1, 2], [2, 3], [3, 0]]), 0, 3,
        vec![(NodeId(0), 0), (NodeId(1), 1), (NodeId(3), 1), (NodeId(2), 2)]
    )]
    #[case::disconnected(
        Graph::new(3, &[[0, 1]]), 0, 3,
        vec![(NodeId(0), 0), (NodeId(1), 1)]
    ...
    #[case::zero(
        Graph::new(3, &[[0, 1], [1, 2]]), 1, 0,
        vec![(NodeId(1), 0)]
    )]
    #[case::maximum(
        Graph::new(3, &[[0, 1], [1, 2]]), 0, usize::MAX,
        vec![(NodeId(0), 0), (NodeId(1), 1), (NodeId(2), 2)]
    )]
    fn test_graph_neighborhood(
        ...
        let actual = graph.neighborhood(NodeId(source), max_depth, NeighborhoodAlgorithm::Bfs);
        assert_eq!(actual, expected);
    }
}
