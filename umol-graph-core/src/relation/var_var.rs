//! Two variable-arity factors with independent row offsets: [VarVarBirelationSet].
...
/// Relations with two distinguished variable-arity participant factors.
///
...
/// Each factor uses its own flat buffer and row offsets; either factor may be empty.
    ...
    L2: RelationParticipant,
{
    ...
    /// Either factor may be empty. Each factor extends its own flat buffer and the payload
    /// is moved into storage. Incidence is rebuilt over the entire collection once, including
    /// references from both factors.
    ///
    /// # Semantic properties
    ///
    ...
    /// relation immediately afterward restores the previous set. These laws are exercised against
    /// a row model in `tests/property/relation.rs`.
    pub fn add(&mut self, participants_1: &[L1], participants_2: &[L2], data: D) -> RelationId {
        let id = RelationId(self.count() as u32);
        self.participants_1.extend_from_slice(participants_1);
        self.f1_offsets.push(self.participants_1.len() as u32);
        ...
        self.incidence = Incidence::build(self.count(), |i, out| {
            out.extend(
                self.participants_1(RelationId::from(i))
                    .iter()
                    .map(|p| p.refs()),
            );
            out.extend(
    ...
    /// A nonempty removal compacts both factor buffers, updates their independent offsets,
    ...
    /// or relabeled. Rows with empty factors survive unless explicitly removed. Incidence uses
    /// the new dense relation ids. Empty input returns identity over the original count and leaves
    ...
    pub fn tracked_remove(&mut self, ids: &[RelationId]) -> Compaction<RelationId> {
        ...
        let mut dst = 0;
        let mut participant_dst_1 = 0;
        let mut participant_dst_2 = 0;
        let mut start_1 = 0;
        let mut start_2 = 0;
        for src in 0..self.count() {
            let end_1 = self.f1_offsets[src + 1] as usize;
            let end_2 = self.f2_offsets[src + 1] as usize;
            if compaction
                .removed()
            ...
            {
                self.participants_1
                    .copy_within(start_1..end_1, participant_dst_1);
                participant_dst_1 += end_1 - start_1;
                self.f1_offsets[dst + 1] = participant_dst_1 as u32;
                self.participants_2
                    .copy_within(start_2..end_2, participant_dst_2);
                participant_dst_2 += end_2 - start_2;
                self.f2_offsets[dst + 1] = participant_dst_2 as u32;
        ...
        self.participants_1.truncate(participant_dst_1);
        self.f1_offsets.truncate(dst + 1);
        self.participants_2.truncate(participant_dst_2);
        self.f2_offsets.truncate(dst + 1);
    ...
    /// Payloads move without cloning. Storage rebuilds both packed participant buffers
    /// using independent row ranges, reuses the payload and offset columns, then
    /// rebuilds incidence. Surviving rows are not materialized as participant vectors.
    ///
    ...
    /// # Semantic properties
    ///
    /// Matching removal and restoration recover original rows, ids, and incidence,
    /// including empty factors, factor boundaries, participant order, and multiplicity.
    /// Surviving rows and current payloads are preserved. Reordering saved entries
    /// does not affect the result. Matching identity is a no-op. Public-API properties in
    ...
    pub fn restore(
        ...
        removed: Vec<(RelationId, Vec<L1>, Vec<L2>, D)>,
    ) {
        ...
        let Some((participant_count_1, participant_count_2)) = removed.iter().try_fold(
            (self.participants_1.len(), self.participants_2.len()),
            |(first, second), (_, row_1, row_2, _)| {
                Some((
                    first.checked_add(row_1.len())?,
                    second.checked_add(row_2.len())?,
                ))
            },
        ) else {
            return;
        ...
        if compaction.source_count() > u32::MAX as usize
            ...
            || participant_count_1 > u32::MAX as usize
            || participant_count_2 > u32::MAX as usize
        {
            return;
        }
        ...
        if rows.try_reserve_exact(count).is_err()
            || participants_1
                .try_reserve_exact(participant_count_1)
                .is_err()
            || participants_2
                .try_reserve_exact(participant_count_2)
                .is_err()
            || self
                .participants_1
                .try_reserve(participant_count_1 - self.participants_1.len())
                .is_err()
            || self
                .participants_2
                .try_reserve(participant_count_2 - self.participants_2.len())
                .is_err()
            || self.f1_offsets.try_reserve(removed.len()).is_err()
        ...
        let mut original = 0;
        for ((data, range_1), range_2) in self
            .data
            .drain(..)
            .zip(self.f1_offsets.windows(2))
        ...
        for (id, first, second, data) in removed {
            if let Some(slot) = rows.get_mut(id.index()) {
                let start_1 = self.participants_1.len();
                let start_2 = self.participants_2.len();
                self.participants_1.extend_from_slice(&first);
                self.participants_2.extend_from_slice(&second);
                *slot = Some((
                    start_1,
                    self.participants_1.len(),
                    start_2,
                    self.participants_2.len(),
                    data,
                ));
            }
        }
        self.f1_offsets.clear();
        self.f1_offsets.push(0);
        self.f2_offsets.clear();
        self.f2_offsets.push(0);
        for (start_1, end_1, start_2, end_2, data) in rows.into_iter().flatten() {
            participants_1.extend_from_slice(&self.participants_1[start_1..end_1]);
            participants_2.extend_from_slice(&self.participants_2[start_2..end_2]);
            self.f1_offsets.push(participants_1.len() as u32);
            self.f2_offsets.push(participants_2.len() as u32);
            self.data.push(data);
        }
    ...
    /// Offsets and payloads are unchanged. Restore participant references before restoring
    /// removed rows, whose saved participants already use original graph ids.
    ///
    ...
    /// empty storage are no-ops. Public-API properties in `tests/property/restore.rs`
    /// check independent inverse-reference expectations and incidence, roundtrips,
    ...
    pub fn restore_participants(&mut self, compaction: &GraphCompaction) {
        ...
        for participant in &mut self.participants_1 {
        ...
        for participant in &mut self.participants_2 {
            let refs = participant.refs();
            if refs
                .node
                .is_some_and(|id| id.index() >= compaction.nodes().result_count())
    ...
    /// without checking membership in an external graph. Both factors may change length,
    /// including both becoming empty. Resizing moves later participants and adjusts offsets
    /// independently in each buffer. Incidence is rebuilt over the entire collection once,
    ...
    pub fn replace_participants(
        ...
        let start_1 = self.f1_offsets[id.index()] as usize;
        let end_1 = self.f1_offsets[id.index() + 1] as usize;
        let old_len_1 = end_1 - start_1;
        let start_2 = self.f2_offsets[id.index()] as usize;
        let end_2 = self.f2_offsets[id.index() + 1] as usize;
        let old_len_2 = end_2 - start_2;
        self.participants_1
            .splice(start_1..end_1, participants_1.iter().copied());
        for offset in &mut self.f1_offsets[id.index() + 1..] {
            *offset = (*offset as usize - old_len_1 + participants_1.len()) as u32;
        }
        self.participants_2
            .splice(start_2..end_2, participants_2.iter().copied());
        for offset in &mut self.f2_offsets[id.index() + 1..] {
            *offset = (*offset as usize - old_len_2 + participants_2.len()) as u32;
        }
        self.incidence = Incidence::build(self.count(), |i, out| {
            out.extend(
                self.participants_1(RelationId::from(i))
    ...
    ///
    /// Panics if `id` is outside the set.
    pub fn replace_participants_1(&mut self, id: RelationId, participants: &[L1]) {
    ...
    ///
    /// Delegates to [`Self::replace_participants`] with a copy of the current first factor,
    /// sharing its admission, preservation, and incidence contract. The replacement may be empty.
    ...
    pub fn replace_participants_2(&mut self, id: RelationId, participants: &[L2]) {
        let first = self.participants_1(id).to_vec();
        self.replace_participants(id, &first, participants);
    }

    /// Replace one participant at a position local to relation `id`'s first factor.
    ///
    /// Delegates to [`Self::replace_participants_1`] with the edited sequence.
    ///
    /// # Semantic properties
    ///
    /// Equivalent to whole-factor replacement after changing only `position`.
    /// The factor length, every other position, and the complete second factor remain unchanged.
    ///
    /// # Panics
    ///
    /// Panics if `id` is outside the set or `position` is outside the first factor.
    /// Every position is invalid for an empty first factor.
    pub fn replace_participant_1(
        ...
        let mut participants = self.participants_1(id).to_vec();
    ...
    /// Insert a first-factor participant before `position`, or append at the factor length.
    ///
    /// Delegates to [`Self::replace_participants_1`] with the edited sequence, sharing its
    /// admission and preservation contract and its full incidence rebuild.
    ///
    /// # Semantic properties
    ///
    /// The first factor grows by one and existing participants retain their relative order.
    /// The second factor is unchanged. Removing the inserted position restores the original set.
    ///
    /// # Panics
    ///
    /// Panics if `id` is outside the set or `position` exceeds the first factor's length.
    pub fn insert_participant_1(
        &mut self,
        id: RelationId,
        position: ParticipantPosition,
        participant: L1,
    ...
    /// Remove the participant at a position local to relation `id`'s first factor.
    ///
    /// Delegates to [`Self::replace_participants_1`] with the edited sequence, sharing its
    /// admission and preservation contract and its full incidence rebuild.
    ///
    /// # Semantic properties
    ///
    /// The first factor shrinks by one and surviving participants retain their relative order.
    /// Removing its sole participant leaves an empty first factor with the same id and payload.
    /// The second factor is unchanged.
    ///
    /// # Panics
    ///
    /// Panics if `id` is outside the set or `position` is outside the first factor.
    /// Every position is invalid for an empty first factor.
    pub fn remove_participant_1(&mut self, id: RelationId, position: ParticipantPosition) {
        let mut participants = self.participants_1(id).to_vec();
