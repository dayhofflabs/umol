...
#[rstest]
#[case::interleaved(
    vec![([NodeId(1), NodeId(1)], vec![], "changed")], 3,
    vec![RelationId(0), RelationId(2)],
    vec![(RelationId(2), [NodeId(4), NodeId(4)], vec![NodeId(1), NodeId(4), NodeId(1)], "last"), (RelationId(0), [NodeId(0), NodeId(1)], vec![NodeId(0)], "first")],
    vec![([NodeId(0), NodeId(1)], vec![NodeId(0)], "first"), ([NodeId(1), NodeId(1)], vec![], "changed"), ([NodeId(4), NodeId(4)], vec![NodeId(1), NodeId(4), NodeId(1)], "last")],
)]
#[case::all(vec![], 2, vec![RelationId(0), RelationId(1)],
    vec![(RelationId(1), [NodeId(1)], vec![], "second"), (RelationId(0), [NodeId(0)], vec![NodeId(1), NodeId(0)], "first")],
    vec![([NodeId(0)], vec![NodeId(1), NodeId(0)], "first"), ([NodeId(1)], vec![], "second")])]
#[case::empty_first(vec![([], vec![NodeId(1)], "survivor")], 2, vec![RelationId(0)],
    vec![(RelationId(0), [], vec![NodeId(0)], "restored")], vec![([], vec![NodeId(0)], "restored"), ([], vec![NodeId(1)], "survivor")])]
#[case::empty_second(vec![([NodeId(1)], vec![], "survivor")], 2, vec![RelationId(1)],
    vec![(RelationId(1), [NodeId(0)], vec![], "restored")], vec![([NodeId(1)], vec![], "survivor"), ([NodeId(0)], vec![], "restored")])]
#[case::empty_both(vec![], 1, vec![RelationId(0)], vec![(RelationId(0), [], vec![], "restored")], vec![([], vec![], "restored")])]
#[case::coincident(vec![([NodeId(1)], vec![NodeId(1)], "survivor")], 2, vec![RelationId(0)],
    vec![(RelationId(0), [NodeId(1)], vec![NodeId(1)], "restored")], vec![([NodeId(1)], vec![NodeId(1)], "restored"), ([NodeId(1)], vec![NodeId(1)], "survivor")])]
fn test_fixed_var_birelation_set_restore<const N1: usize>(
    #[case] entries: Vec<([NodeId; N1], Vec<NodeId>, &'static str)>,
    #[case] count: usize,
    #[case] ids: Vec<RelationId>,
    #[case] removed: Vec<(RelationId, [NodeId; N1], Vec<NodeId>, &'static str)>,
...
#[rstest]
#[case::empty(FixedVarBirelationSet::<NodeId, 1, EdgeId, &str>::new(vec![]))]
#[case::empty_first(FixedVarBirelationSet::new(vec![([], vec![EdgeId(0)], "first")]))]
#[case::empty_second(FixedVarBirelationSet::new(vec![([NodeId(0)], vec![], "first")]))]
#[case::empty_both(FixedVarBirelationSet::new(vec![([], vec![], "first")]))]
#[case::nonempty(FixedVarBirelationSet::new(vec![([NodeId(0)], vec![EdgeId(2), EdgeId(2)], "first")]))]
fn test_fixed_var_birelation_set_restore_identity<const N1: usize>(
    #[case] input: FixedVarBirelationSet<NodeId, N1, EdgeId, &'static str>,
) {
    let mut relations = input.clone();
...
#[rstest]
#[case::mixed([NodeId(2), NodeId(2)], vec![EdgeId(0)], [NodeId(0), NodeId(2)], vec![EdgeId(2)])]
#[case::empty_first([], vec![EdgeId(0)], [], vec![EdgeId(2)])]
#[case::empty_second([NodeId(2)], vec![], [NodeId(0)], vec![])]
#[case::empty_both([], vec![], [], vec![])]
fn test_fixed_var_birelation_set_restore_payload<const N1: usize>(
    #[case] first: [NodeId; N1],
...
#[rstest]
#[case::mixed(vec![([NodeId(0), NodeId(1)], vec![EdgeId(1)], "first"), ([NodeId(2), NodeId(0)], vec![], "second")],
    vec![([NodeId(1), NodeId(3)], vec![EdgeId(2)], "first"), ([NodeId(4), NodeId(1)], vec![], "second")])]
#[case::shared_nodes(vec![([NodeId(0)], vec![NodeId(0), NodeId(1)], "first")], vec![([NodeId(1)], vec![NodeId(1), NodeId(3)], "first")])]
#[case::shared_edges(vec![([EdgeId(1)], vec![EdgeId(1), EdgeId(0)], "first")], vec![([EdgeId(2)], vec![EdgeId(2), EdgeId(0)], "first")])]
#[case::empty_first(vec![([] as [NodeId; 0], vec![EdgeId(1)], "first")], vec![([], vec![EdgeId(2)], "first")])]
#[case::empty_second(vec![([NodeId(0)], Vec::<EdgeId>::new(), "first")], vec![([NodeId(1)], vec![], "first")])]
#[case::empty_both(vec![([] as [NodeId; 0], Vec::<EdgeId>::new(), "first")], vec![([], vec![], "first")])]
#[case::empty(Vec::<([NodeId; 1], Vec<EdgeId>, &str)>::new(), vec![])]
fn test_fixed_var_birelation_set_restore_participants<
    L1: RelationParticipant + Debug,
    const N1: usize,
    L2: RelationParticipant + Debug,
>(
    #[case] entries: Vec<([L1; N1], Vec<L2>, &'static str)>,
    #[case] expected: Vec<([L1; N1], Vec<L2>, &'static str)>,
) {
    let mut relations = FixedVarBirelationSet::new(entries);
...
#[rstest]
fn test_fixed_var_birelation_set_restore_participants_references() {
    let mut relations = FixedVarBirelationSet::default();
    relations.add(
        [
            References {
...
#[case::empty_both(FixedVarBirelationSet::new(vec![([], vec![], "first")]))]
#[case::nonempty(FixedVarBirelationSet::new(vec![([NodeId(0)], vec![EdgeId(2), EdgeId(2)], "first")]))]
fn test_fixed_var_birelation_set_restore_participants_identity<const N1: usize>(
    #[case] input: FixedVarBirelationSet<NodeId, N1, EdgeId, &'static str>,
) {
    let mut relations = input.clone();
    relations.restore_participants(&GraphCompaction::new(
