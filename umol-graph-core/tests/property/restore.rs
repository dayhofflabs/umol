...
//!
//! FixedVarBirelationSet uses a two-participant first factor and variable second factors
//! of zero to eight participants, checking row boundaries and incidence across both factors.
//!
...
use umol_graph_core::{
    ...
    GraphCompaction, GraphCorrespondence, GraphRemapping, Neighbor, NodeId, ParticipantRefs,
    RelationId, RelationParticipant, VarRelationSet, VarVarBirelationSet,
};
...
proptest! {
    ...
    fn test_fixed_var_birelation_set_restore(
        ...
        let compaction = Compaction::new(rows.len(), ids).unwrap();
        let expected: Vec<_> = rows.iter().enumerate().map(|(id, (row, remove))| (
            [NodeId(row.0[0]), NodeId(row.0[1])], row.1.iter().copied().map(NodeId).collect::<Vec<_>>(),
            if *remove { id } else { id + rows.len() })).collect();
        let survivors = expected.iter().zip(&rows).filter(|(_, (_, remove))| !remove).map(|(entry, _)| entry.clone()).collect();
        let mut saved: Vec<_> = expected.iter().zip(&rows).enumerate().rev().filter(|(_, (_, (_, remove)))| *remove)
            .map(|(id, ((first, second, data), _))| (RelationId::from(id), *first, second.clone(), *data)).collect();
        saved.sort_unstable_by_key(|(id, _, _, _)| (order[id.index()], *id));
        let mut relations = FixedVarBirelationSet::new(survivors);
        relations.restore(&compaction, saved);
        for node in 0..17 {
            let incidence: Vec<_> = expected.iter().enumerate()
                .filter(|(_, (first, second, _))| first.contains(&NodeId(node)) || second.contains(&NodeId(node)))
    ...
    #[test]
    fn test_fixed_var_birelation_set_restore_roundtrip(
        rows in prop::collection::vec(((prop::array::uniform2(0u32..16), prop::collection::vec(0u32..16, 0..9)), any::<bool>()), 0..33),
    ) {
        let expected: Vec<_> = rows.iter().enumerate().map(|(id, (row, _))| (
            [NodeId(row.0[0]), NodeId(row.0[1])], row.1.iter().copied().map(EdgeId).collect::<Vec<_>>(), id)).collect();
        let original = FixedVarBirelationSet::new(expected.clone());
        let ids: Vec<_> = rows.iter().enumerate().filter(|(_, (_, remove))| *remove).map(|(id, _)| RelationId::from(id)).collect();
        let saved = expected.iter().zip(&rows).enumerate().rev().filter(|(_, (_, (_, remove)))| *remove)
            .map(|(id, ((first, second, data), _))| (RelationId::from(id), *first, second.clone(), *data)).collect();
        let mut relations = original.clone();
    ...
    fn test_fixed_var_birelation_set_restore_malformed(
        rows in prop::collection::vec((prop::array::uniform2(any::<u32>()), prop::collection::vec(any::<u32>(), 0..9)), 0..33),
        count in 0usize..41,
        removals in prop::collection::vec(any::<bool>(), 40),
        saved in prop::collection::vec((0u32..48, (prop::array::uniform2(any::<u32>()), prop::collection::vec(any::<u32>(), 0..9))), 0..41),
    ) {
        let mut relations = FixedVarBirelationSet::new(rows.into_iter().enumerate().map(|(id, row)| (
            [NodeId(row.0[0]), NodeId(row.0[1])], row.1.iter().copied().map(EdgeId).collect::<Vec<_>>(), id)).collect());
        let compaction = Compaction::new(count, (0..count).filter(|&id| removals[id]).map(RelationId::from).collect()).unwrap();
        relations.restore(&compaction, saved.into_iter().enumerate().map(|(data, (id, row))| (
            RelationId(id), [NodeId(row.0[0]), NodeId(row.0[1])], row.1.iter().copied().map(EdgeId).collect::<Vec<_>>(), data)).collect());
    }

    #[test]
    fn test_fixed_var_birelation_set_restore_participants(
        nodes in 0usize..9,
        edges in 0usize..9,
        node_removals in prop::collection::vec(any::<bool>(), 8),
        edge_removals in prop::collection::vec(any::<bool>(), 8),
        rows in prop::collection::vec((prop::array::uniform2((0usize..8, 0usize..8, any::<bool>(), any::<bool>(), any::<u8>())), prop::collection::vec((0usize..8, 0usize..8, any::<bool>(), any::<bool>(), any::<u8>()), 0..9)), 0..33),
    ) {
        let surviving_nodes: Vec<_> = (0..nodes).filter(|&id| !node_removals[id]).map(NodeId::from).collect();
        let surviving_edges: Vec<_> = (0..edges).filter(|&id| !edge_removals[id]).map(EdgeId::from).collect();
        let compaction = GraphCompaction::new(
        ...
        let entries: Vec<_> = rows.iter().enumerate().map(|(id, row)| {
            let first = row.0.map(|(n, e, has_n, has_e, label)| References {
                node: (has_n && !surviving_nodes.is_empty()).then(|| NodeId::from(n % surviving_nodes.len())),
                edge: (has_e && !surviving_edges.is_empty()).then(|| EdgeId::from(e % surviving_edges.len())), label,
            });
            let second = row.1.iter().copied().map(|(n, e, has_n, has_e, label)| References {
                node: (has_n && !surviving_nodes.is_empty()).then(|| NodeId::from(n % surviving_nodes.len())),
                edge: (has_e && !surviving_edges.is_empty()).then(|| EdgeId::from(e % surviving_edges.len())), label,
            }).collect::<Vec<_>>();
            (first, second, id)
        ...
        let expected: Vec<_> = entries.iter().map(|(first, second, data)| (
            first.map(|p| References { node: p.node.map(|id| surviving_nodes[id.index()]), edge: p.edge.map(|id| surviving_edges[id.index()]), ..p }),
            second.iter().copied().map(|p| References { node: p.node.map(|id| surviving_nodes[id.index()]), edge: p.edge.map(|id| surviving_edges[id.index()]), ..p }).collect::<Vec<_>>(), *data)).collect();
    ...
    #[test]
    fn test_fixed_var_birelation_set_restore_participants_roundtrip(
        rows in prop::collection::vec((prop::array::uniform2((0u32..8, 0u32..8, any::<bool>(), any::<bool>(), any::<u8>())), prop::collection::vec((0u32..8, 0u32..8, any::<bool>(), any::<bool>(), any::<u8>()), 0..9)), 0..33),
        node_removals in prop::collection::vec(any::<bool>(), 8),
        edge_removals in prop::collection::vec(any::<bool>(), 8),
    ) {
        let entries: Vec<_> = rows.into_iter().enumerate().map(|(id, row)| {
            let first = row.0.map(|(n, e, has_n, has_e, label)| References { node: has_n.then_some(NodeId(n)), edge: has_e.then_some(EdgeId(e)), label });
            let second = row.1.iter().copied().map(|(n, e, has_n, has_e, label)| References { node: has_n.then_some(NodeId(n)), edge: has_e.then_some(EdgeId(e)), label }).collect::<Vec<_>>();
            (first, second, id)
        }).collect();
        let expected: Vec<_> = entries.iter().filter(|(first, second, _)| first.iter().chain(second).all(|p|
            p.node.is_none_or(|id| !node_removals[id.index()]) && p.edge.is_none_or(|id| !edge_removals[id.index()]))).cloned().collect();
        let compaction = GraphCompaction::new(
            Compaction::new(8, (0..8).filter(|&id| node_removals[id]).map(NodeId::from).collect()).unwrap(),
            Compaction::new(8, (0..8).filter(|&id| edge_removals[id]).map(EdgeId::from).collect()).unwrap(),
        );
        let mut relations = FixedVarBirelationSet::new(entries).compact(&compaction);
        relations.restore_participants(&compaction);
        for id in 0..9 {
            let nodes: Vec<_> = expected.iter().enumerate().filter(|(_, (first, second, _))|
                first.iter().chain(second).any(|p| p.node == Some(NodeId(id)))).map(|(i, _)| RelationId::from(i)).collect();
    ...
    fn test_fixed_var_birelation_set_restore_participants_malformed(
        rows in prop::collection::vec((prop::array::uniform2((prop::option::of(0u32..16), prop::option::of(0u32..16), any::<u8>())), prop::collection::vec((prop::option::of(0u32..16), prop::option::of(0u32..16), any::<u8>()), 0..9)), 0..33),
        nodes in 0usize..9,
        edges in 0usize..9,
        node_removals in prop::collection::vec(any::<bool>(), 8),
        edge_removals in prop::collection::vec(any::<bool>(), 8),
        ...
        let mut relations = FixedVarBirelationSet::new(rows.into_iter().enumerate().map(|(id, row)| {
            let first = row.0.map(|(node, edge, label)| References { node: node.map(NodeId), edge: edge.map(EdgeId), label });
            let second = row.1.iter().copied().map(|(node, edge, label)| References { node: node.map(NodeId), edge: edge.map(EdgeId), label }).collect::<Vec<_>>();
            (first, second, id)
        }).collect());
        relations.restore_participants(&GraphCompaction::new(
            Compaction::new(nodes, (0..nodes).filter(|&id| node_removals[id]).map(NodeId::from).collect()).unwrap(),
    ...
    fn test_var_var_birelation_set_restore(
        ...
        let compaction = Compaction::new(rows.len(), ids).unwrap();
        let expected: Vec<_> = rows.iter().enumerate().map(|(id, (row, remove))| (
            row.0.iter().copied().map(NodeId).collect::<Vec<_>>(), row.1.iter().copied().map(NodeId).collect::<Vec<_>>(),
            if *remove { id } else { id + rows.len() })).collect();
        let survivors = expected.iter().zip(&rows).filter(|(_, (_, remove))| !remove).map(|(entry, _)| entry.clone()).collect();
        let mut saved: Vec<_> = expected.iter().zip(&rows).enumerate().rev().filter(|(_, (_, (_, remove)))| *remove)
            .map(|(id, ((first, second, data), _))| (RelationId::from(id), first.clone(), second.clone(), *data)).collect();
        saved.sort_unstable_by_key(|(id, _, _, _)| (order[id.index()], *id));
        let mut relations = VarVarBirelationSet::new(survivors);
        relations.restore(&compaction, saved);
        for node in 0..17 {
            let incidence: Vec<_> = expected.iter().enumerate()
                .filter(|(_, (first, second, _))| first.contains(&NodeId(node)) || second.contains(&NodeId(node)))
    ...
    #[test]
    fn test_var_var_birelation_set_restore_roundtrip(
        rows in prop::collection::vec(((prop::collection::vec(0u32..16, 0..9), prop::collection::vec(0u32..16, 0..9)), any::<bool>()), 0..33),
    ) {
        let expected: Vec<_> = rows.iter().enumerate().map(|(id, (row, _))| (
            row.0.iter().copied().map(NodeId).collect::<Vec<_>>(), row.1.iter().copied().map(EdgeId).collect::<Vec<_>>(), id)).collect();
        let original = VarVarBirelationSet::new(expected.clone());
        let ids: Vec<_> = rows.iter().enumerate().filter(|(_, (_, remove))| *remove).map(|(id, _)| RelationId::from(id)).collect();
        let saved = expected.iter().zip(&rows).enumerate().rev().filter(|(_, (_, (_, remove)))| *remove)
            .map(|(id, ((first, second, data), _))| (RelationId::from(id), first.clone(), second.clone(), *data)).collect();
        let mut relations = original.clone();
    ...
    fn test_var_var_birelation_set_restore_malformed(
        rows in prop::collection::vec((prop::collection::vec(any::<u32>(), 0..9), prop::collection::vec(any::<u32>(), 0..9)), 0..33),
        count in 0usize..41,
        removals in prop::collection::vec(any::<bool>(), 40),
        saved in prop::collection::vec((0u32..48, (prop::collection::vec(any::<u32>(), 0..9), prop::collection::vec(any::<u32>(), 0..9))), 0..41),
    ) {
        let mut relations = VarVarBirelationSet::new(rows.into_iter().enumerate().map(|(id, row)| (
            row.0.iter().copied().map(NodeId).collect::<Vec<_>>(), row.1.iter().copied().map(EdgeId).collect::<Vec<_>>(), id)).collect());
        let compaction = Compaction::new(count, (0..count).filter(|&id| removals[id]).map(RelationId::from).collect()).unwrap();
        relations.restore(&compaction, saved.into_iter().enumerate().map(|(data, (id, row))| (
            RelationId(id), row.0.iter().copied().map(NodeId).collect::<Vec<_>>(), row.1.iter().copied().map(EdgeId).collect::<Vec<_>>(), data)).collect());
    }
    ...
    fn test_var_var_birelation_set_restore_participants(
        ...
        node_removals in prop::collection::vec(any::<bool>(), 8),
        edge_removals in prop::collection::vec(any::<bool>(), 8),
        rows in prop::collection::vec((prop::collection::vec((0usize..8, 0usize..8, any::<bool>(), any::<bool>(), any::<u8>()), 0..9), prop::collection::vec((0usize..8, 0usize..8, any::<bool>(), any::<bool>(), any::<u8>()), 0..9)), 0..33),
    ) {
        let surviving_nodes: Vec<_> = (0..nodes).filter(|&id| !node_removals[id]).map(NodeId::from).collect();
        let surviving_edges: Vec<_> = (0..edges).filter(|&id| !edge_removals[id]).map(EdgeId::from).collect();
        let compaction = GraphCompaction::new(
        ...
        let entries: Vec<_> = rows.iter().enumerate().map(|(id, row)| {
            let first = row.0.iter().copied().map(|(n, e, has_n, has_e, label)| References {
                node: (has_n && !surviving_nodes.is_empty()).then(|| NodeId::from(n % surviving_nodes.len())),
                edge: (has_e && !surviving_edges.is_empty()).then(|| EdgeId::from(e % surviving_edges.len())), label,
            }).collect::<Vec<_>>();
            let second = row.1.iter().copied().map(|(n, e, has_n, has_e, label)| References {
        ...
        let expected: Vec<_> = entries.iter().map(|(first, second, data)| (
            first.iter().copied().map(|p| References { node: p.node.map(|id| surviving_nodes[id.index()]), edge: p.edge.map(|id| surviving_edges[id.index()]), ..p }).collect::<Vec<_>>(),
            second.iter().copied().map(|p| References { node: p.node.map(|id| surviving_nodes[id.index()]), edge: p.edge.map(|id| surviving_edges[id.index()]), ..p }).collect::<Vec<_>>(), *data)).collect();
    ...
    #[test]
    fn test_var_var_birelation_set_restore_participants_roundtrip(
        rows in prop::collection::vec((prop::collection::vec((0u32..8, 0u32..8, any::<bool>(), any::<bool>(), any::<u8>()), 0..9), prop::collection::vec((0u32..8, 0u32..8, any::<bool>(), any::<bool>(), any::<u8>()), 0..9)), 0..33),
        node_removals in prop::collection::vec(any::<bool>(), 8),
        edge_removals in prop::collection::vec(any::<bool>(), 8),
    ) {
        let entries: Vec<_> = rows.into_iter().enumerate().map(|(id, row)| {
            let first = row.0.iter().copied().map(|(n, e, has_n, has_e, label)| References { node: has_n.then_some(NodeId(n)), edge: has_e.then_some(EdgeId(e)), label }).collect::<Vec<_>>();
            let second = row.1.iter().copied().map(|(n, e, has_n, has_e, label)| References { node: has_n.then_some(NodeId(n)), edge: has_e.then_some(EdgeId(e)), label }).collect::<Vec<_>>();
            (first, second, id)
        }).collect();
        let expected: Vec<_> = entries.iter().filter(|(first, second, _)| first.iter().chain(second).all(|p|
        ...
        let mut relations = VarVarBirelationSet::new(entries).compact(&compaction);
        relations.restore_participants(&compaction);
        for id in 0..9 {
            let nodes: Vec<_> = expected.iter().enumerate().filter(|(_, (first, second, _))|
                first.iter().chain(second).any(|p| p.node == Some(NodeId(id)))).map(|(i, _)| RelationId::from(i)).collect();
    ...
    fn test_var_var_birelation_set_restore_participants_malformed(
        rows in prop::collection::vec((prop::collection::vec((prop::option::of(0u32..16), prop::option::of(0u32..16), any::<u8>()), 0..9), prop::collection::vec((prop::option::of(0u32..16), prop::option::of(0u32..16), any::<u8>()), 0..9)), 0..33),
        nodes in 0usize..9,
        edges in 0usize..9,
        node_removals in prop::collection::vec(any::<bool>(), 8),
        edge_removals in prop::collection::vec(any::<bool>(), 8),
        ...
        let mut relations = VarVarBirelationSet::new(rows.into_iter().enumerate().map(|(id, row)| {
            let first = row.0.iter().copied().map(|(node, edge, label)| References { node: node.map(NodeId), edge: edge.map(EdgeId), label }).collect::<Vec<_>>();
            let second = row.1.iter().copied().map(|(node, edge, label)| References { node: node.map(NodeId), edge: edge.map(EdgeId), label }).collect::<Vec<_>>();
            (first, second, id)
        }).collect());
        relations.restore_participants(&GraphCompaction::new(
