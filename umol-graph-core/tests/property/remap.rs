...
//!
//! The reorder law is checked for shuffled permutations of zero through 64 entries.
...
proptest! {
    ...
    fn test_remapping_reorder(
        ...
        prop_assert_eq!(remapping.reorder(values.clone()), expected.clone());
        prop_assert_eq!(remapping.try_reorder(values), Some(expected));
    }
}
