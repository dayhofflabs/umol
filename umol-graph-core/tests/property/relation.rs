...
//! [VarRelationSet::add] and [VarRelationSet::tracked_remove] additionally exercise mixed
//! row additions, replacements, and removals with independently varying participant and payload
//! lengths. Direct row scans check empty-row preservation, compacted offsets, and incidence;
//! a separate add/remove roundtrip checks restoration after appending an empty or nonempty row.
...
//! [FixedVarBirelationSet::add] and [FixedVarBirelationSet::tracked_remove] exercise mixed
//! addition, replacement, and removal against independent rows. Empty variable factors and
//! changing lengths check alignment with the fixed column and payloads through public readers.
//! Direct union scans check compacted incidence; a separate add/remove roundtrip checks restoration.
...
//! [VarVarBirelationSet::add] and [VarVarBirelationSet::tracked_remove] compare mixed
...
#[rstest]
fn test_var_relation_set_add_roundtrip() {
    let strategy = (
        prop::collection::vec(
            (
    ...
    let config = Config {
        ...
        test_name: Some(concat!(
            module_path!(),
            "::test_var_relation_set_add_roundtrip"
        )),
        ..Config::default()
    };
    TestRunner::new(config)
        .run(&strategy, |(entries, participants, data)| {
            ...
            let id = relations.add(&participants, data.clone());
            prop_assert_eq!(id, RelationId::from(entries.len()));
            let mut appended = entries.clone();
            appended.push((participants.clone(), data));
            assert_var_relation_rows(&relations, &appended, &[], &participants)?;
            relations.remove(&[id]);
...
fn test_var_relation_set_tracked_remove() {
    ...
    let config = Config {
        ...
        test_name: Some(concat!(
            module_path!(),
            "::test_var_relation_set_tracked_remove"
        )),
        ..Config::default()
    };
    TestRunner::new(config)
            ...
            for (operation, participants, data, position, removals) in edits {
                ...
                match operation {
                    0 => {
                        let id = relations.add(&participants, data.clone());
                        prop_assert_eq!(id, RelationId::from(entries.len()));
                        entries.push((participants.clone(), data));
                    }
                    1 if !entries.is_empty() => {
                        let row = position % entries.len();
                        relations.replace_participants(RelationId::from(row), &participants);
                        entries[row].0 = participants.clone();
                    }
                    _ => {
                        let ids: Vec<_> = if entries.is_empty() {
                            vec![]
                ...
                assert_var_relation_rows(&relations, &entries, &previous, &participants)?;
            }
            prop_assert_eq!(relations.into_entries(), entries);
            Ok(())
        })
...
#[rstest]
fn test_fixed_var_birelation_set_add_roundtrip() {
    let strategy = (
        prop::collection::vec(
            (
                prop::array::uniform2(participant_strategy()),
    ...
    let config = Config {
        ...
        test_name: Some(concat!(
            module_path!(),
            "::test_fixed_var_birelation_set_add_roundtrip"
        )),
        ..Config::default()
    };
    TestRunner::new(config)
        .run(&strategy, |(entries, first, second, data)| {
            ...
            let query = (first.to_vec(), second.to_vec());
            let mut appended = entries.clone();
            appended.push((first, second.clone(), data));
...
fn test_fixed_var_birelation_set_tracked_remove() {
    ...
    let config = Config {
        ...
        test_name: Some(concat!(
            module_path!(),
            "::test_fixed_var_birelation_set_tracked_remove"
        )),
        ..Config::default()
    };
    TestRunner::new(config)
...
#[rstest]
fn test_var_var_birelation_set_add_roundtrip() {
    let strategy = (
        prop::collection::vec(
            (
                prop::collection::vec(participant_strategy(), 0..7),
    ...
    let config = Config {
        ...
        test_name: Some(concat!(
            module_path!(),
            "::test_var_var_birelation_set_add_roundtrip"
        )),
        ..Config::default()
    };
    TestRunner::new(config)
        .run(&strategy, |(entries, first, second, data)| {
            ...
            let query = (first.to_vec(), second.to_vec());
            let mut appended = entries.clone();
            appended.push((first.clone(), second.clone(), data));
...
fn test_var_var_birelation_set_tracked_remove() {
    ...
    let config = Config {
        ...
        test_name: Some(concat!(
            module_path!(),
            "::test_var_var_birelation_set_tracked_remove"
        )),
        ..Config::default()
    };
    TestRunner::new(config)
        .run(&strategy, |(mut entries, edits)| {
            ...
            for (operation, first, second, data, position, removals) in edits {
                let previous = entries
                    ...
                    .unwrap_or((first.clone(), second.clone()));
                let query = (first.to_vec(), second.to_vec());
                match operation {
                    0 => {
                        let id = relations.add(&first, &second, data.clone());
