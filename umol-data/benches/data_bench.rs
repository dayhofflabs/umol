...
fn data_benchmarks(c: &mut Criterion) {
...

    {
        ...
        for element in &vec![e!(C), e!(Cu), e!(Zn)] {
...

    {
        ...
        for element in &vec![e!(C), e!(Ne), e!(Og)] {
            group.bench_with_input(element.symbol(), element, |b, element| {
                b.iter(|| {
                    let _ = element.next();
                });
...

    {
        ...
        for symbol in &vec!["12C".to_string(), "63Cu".to_string(), "209Bi".to_string()] {
            group.bench_with_input(symbol, symbol, |b, symbol| {
                b.iter(|| {
