...
def test_atomview_set_lone_pairs():
    mol = MoleculeAst.from_parts([AtomAst(Element("O"))])
...
def test_atomview_repr():
    mol = MoleculeAst.from_parts([AtomAst(Element("C")), AtomAst(Element("O"))])
    assert repr(mol.atoms[0]) == "AtomView(id=0)"
    assert repr(mol.atoms) == "AtomViews(len=2)"


def test_atomview_asdict():
    mol = MoleculeAst.from_parts([AtomAst(Element("C"), charge=ValueAst.Lit(-1))])
