//! SDF file classifier for conformance test organization.
//!
//! Classifies SDF files into categories based on which parser configurations succeed:
...
//!
//! Usage:
//!   cargo run --bin classify_sdf_files           # Show classification stats
//!   cargo run --bin classify_sdf_files -- --sort # Copy files to data/ directories
...
use umol_io::ctfile::parser::{parse_extended_sdf_bytes_with, parse_sdf_bytes_with};
...
fn classify_sdf_file(file_path: &Path) -> Result<(Category, ParseResults), Box<dyn Error>> {
    let sdf_bytes = fs::read(file_path)?;
    ...
    let results = ParseResults {
        mol_basic: parse_sdf_bytes_with(&sdf_bytes, &mol_basic_config).is_ok(),
        mol_lenient: parse_sdf_bytes_with(&sdf_bytes, &mol_lenient_config).is_ok(),
        ext_extended: parse_extended_sdf_bytes_with(&sdf_bytes, &ext_extended_config).is_ok(),
        ext_lenient: parse_extended_sdf_bytes_with(&sdf_bytes, &ext_lenient_config).is_ok(),
    };
...
fn collect_sdf_files(dir: &Path) -> Result<Vec<PathBuf>, Box<dyn Error>> {
    ...
    for entry in fs::read_dir(dir)? {
        ...
        if path.is_dir() {
            files.extend(collect_sdf_files(&path)?);
        } else if path.extension() == Some(OsStr::new("sdf")) {
            files.push(path);
        }
    }
    Ok(files)
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    if !(args.len() == 1 || args.len() == 2 && args[1] == "--sort") {
        eprintln!(
            "Usage: cargo run --bin classify_sdf_files           # Show classification stats"
        );
        eprintln!(
            "       cargo run --bin classify_sdf_files -- --sort # Copy files to data/ directories"
        );
    ...
    let data_raw_path = format!("{}/tests/sdf_parsing/data_raw", base_path);
    let data_path = format!("{}/tests/sdf_parsing/data", base_path);
    ...
    for entry in fs::read_dir(&data_raw_path)? {
        ...
        let sdf_files = collect_sdf_files(&source_path)?;

        for path in sdf_files {
            total_files += 1;

            match classify_sdf_file(&path) {
                Ok((category, results)) => {
                    stats.add(category);
    ...
    println!("# SDF File Classification Results\n");
    ...
    if !should_sort {
        ...
        println!("```");
        println!("cargo run --bin classify_sdf_files -- --sort");
        println!("```");
    } else {
