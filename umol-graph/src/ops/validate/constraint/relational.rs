...
use umol_graph_ir::ir::{
    AtomConstraintForm, AtomId, Entity, Lattice, Molecule, RelationalConstraint, RingConfig,
    RingModel, RingViews,
};
...
pub struct RelationalConstraintInvariantsValidator;

#[derive(Clone, Debug, Error, PartialEq, Eq)]
#[error("relational constraint is not satisfied: {constraint:?}")]
pub struct RelationalConstraintInvariantsContradiction {
...
impl RelationalConstraintInvariantsValidator {
    ...
    ) -> Result<Solution<(), RelationalConstraintInvariantsContradiction>, ConstraintInvariantsError>
    {
        ...
        Ok(match truth {
            ...
            Truth::False => Solution::Contradictory(RelationalConstraintInvariantsContradiction {
...
fn require_entity(molecule: &Molecule, entity: Entity) -> Result<(), ConstraintInvariantsError> {
...
fn require_atoms(
    molecule: &Molecule,
    atoms: impl IntoIterator<Item = AtomId>,
) -> Result<(), ConstraintInvariantsError> {
    for atom in atoms {
        require_entity(molecule, Entity::Atom(atom))?;
    }
    Ok(())
...
fn invalid_reference(entity: Entity) -> ConstraintInvariantsError {
    ConstraintInvariantsError::InvalidReference { entity }
}
...
mod tests {
    ...
    use umol_graph_ir::ir::{
        AromaticSystemId, BondId, DativeBondId, MulticenterBondId, NoncovalentBondId, NumForm,
        RingScope, StereoAtomId, StereoBondId,
    };
    ...
    fn test_relational_constraint_validator_validate(
        ...
        assert_eq!(
            RelationalConstraintInvariantsValidator.validate(
                &relational_molecule,
                &constraint,
