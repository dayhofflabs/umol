//! Molecule-scope aggregate and connectivity constraint evaluation.

use std::collections::BTreeSet;

use thiserror::Error;
use umol_graph_core::ConnectedComponentsAlgorithm;
use umol_graph_ir::ir::{AtomId, BondId, Entity, Lattice, Molecule, MoleculeConstraint, NumForm};
use umol_utils::solution::Solution;

use super::{ConstraintInvariantsError, ConstraintValidateConfig};

/// Evaluates one molecule-scope aggregate or connectivity constraint.
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub struct MoleculeConstraintInvariantsValidator;

#[derive(Clone, Debug, Error, PartialEq, Eq)]
#[error("molecule constraint is not satisfied: {constraint:?}")]
pub struct MoleculeConstraintInvariantsContradiction {
    pub constraint: MoleculeConstraint,
}

impl MoleculeConstraintInvariantsValidator {
    /// Validate one molecule-scope constraint using the supplied algorithm choices.
    pub fn validate(
        &self,
        molecule: &Molecule,
        constraint: &MoleculeConstraint,
        config: ConstraintValidateConfig,
    ) -> Result<Solution<(), MoleculeConstraintInvariantsContradiction>, ConstraintInvariantsError>
    {
        let determined = match constraint {
            MoleculeConstraint::ChargeSum { atoms, sum } => {
                let atoms = atom_subset(molecule, atoms.as_deref())?;
                let derived = atoms
                    .into_iter()
                    .map(|atom| molecule.atom(atom).charge())
                    .fold(NumForm::Lit(0), |sum, charge| sum + charge);
                return Ok(evaluate(sum, &derived, constraint));
            }
            MoleculeConstraint::UnpairedElectronCoupling {
                atoms,
                unpaired_electrons,
            } => {
                atom_subset(molecule, atoms.as_deref())?;
                if unpaired_electrons.is_undetermined() {
                    true
                } else {
                    return Ok(Solution::Underdetermined(()));
                }
            }
            MoleculeConstraint::BondOrderSum { bonds, sum } => {
                let bonds = bond_subset(molecule, bonds.as_deref())?;
                let derived = bonds
                    .into_iter()
                    .map(|bond| molecule.bond(bond).order())
                    .fold(NumForm::Lit(0), |sum, order| sum + order);
                return Ok(evaluate(sum, &derived, constraint));
            }
            MoleculeConstraint::Connected { atoms } => {
                let atoms = atom_subset(molecule, atoms.as_deref())?;
                connected(molecule, &atoms, config.connected_components_algorithm)
            }
        };

        Ok(if determined {
            Solution::Determined(())
        } else {
            Solution::Contradictory(MoleculeConstraintInvariantsContradiction {
                constraint: constraint.clone(),
            })
        })
    }
}

fn evaluate(
    asserted: &NumForm,
    derived: &NumForm,
    constraint: &MoleculeConstraint,
) -> Solution<(), MoleculeConstraintInvariantsContradiction> {
    if asserted.is_undetermined() {
        Solution::Determined(())
    } else if !derived.is_ground() {
        Solution::Underdetermined(())
    } else if asserted.matches(derived) {
        Solution::Determined(())
    } else {
        Solution::Contradictory(MoleculeConstraintInvariantsContradiction {
            constraint: constraint.clone(),
        })
    }
}

fn atom_subset(
    molecule: &Molecule,
    atoms: Option<&[AtomId]>,
) -> Result<Vec<AtomId>, ConstraintInvariantsError> {
    match atoms {
        Some(atoms) => {
            let mut selected = BTreeSet::new();
            for &atom in atoms {
                if !molecule.atoms().contains(atom) {
                    return Err(ConstraintInvariantsError::InvalidReference {
                        entity: Entity::Atom(atom),
                    });
                }
                selected.insert(atom);
            }
            Ok(selected.into_iter().collect())
        }
        None => Ok(molecule.atoms().ids().collect()),
    }
}

fn bond_subset(
    molecule: &Molecule,
    bonds: Option<&[BondId]>,
) -> Result<Vec<BondId>, ConstraintInvariantsError> {
    match bonds {
        Some(bonds) => {
            let mut selected = BTreeSet::new();
            for &bond in bonds {
                if !molecule.bonds().contains(bond) {
                    return Err(ConstraintInvariantsError::InvalidReference {
                        entity: Entity::Bond(bond),
                    });
                }
                selected.insert(bond);
            }
            Ok(selected.into_iter().collect())
        }
        None => Ok(molecule.bonds().ids().collect()),
    }
}

/// Whether every selected atom belongs to one localized-bond component. Paths may pass through
/// atoms outside the selected subset; empty and singleton subsets are connected.
fn connected(
    molecule: &Molecule,
    atoms: &[AtomId],
    algorithm: ConnectedComponentsAlgorithm,
) -> bool {
    if atoms.len() < 2 {
        return true;
    }
    let selected: BTreeSet<_> = atoms.iter().copied().collect();
    molecule
        .graph()
        .enumerate_connected_components(algorithm)
        .into_iter()
        .any(|component| {
            component
                .into_iter()
                .filter(|atom| selected.contains(atom))
                .count()
                == atoms.len()
        })
}

#[cfg(test)]
mod tests {
    use rstest::{fixture, rstest};
    use umol_graph_core::RelevantCycleEnumerationAlgorithm;
    use umol_graph_ir::ir::UnpairedElectronsForm;
    use umol_graph_ir::mol_dsl;

    use super::*;

    const CONFIG: ConstraintValidateConfig = ConstraintValidateConfig {
        relevant_cycle_algorithm: RelevantCycleEnumerationAlgorithm::Vismara,
        connected_components_algorithm: ConnectedComponentsAlgorithm::Bfs,
    };

    #[fixture]
    fn aggregate_molecule() -> Molecule {
        mol_dsl!(
            r#"{:atoms ["C#c+" "N#c-" "O#c2" "F#c0" "Cl#c0"]
                :bonds [[0 1 "1"] [1 2 "2"] [3 4 "1"]]}"#
        )
    }

    #[rstest]
    #[case::charge_subset(MoleculeConstraint::ChargeSum {
        atoms: Some(vec![AtomId(0), AtomId(1)]),
        sum: NumForm::Lit(0),
    })]
    #[case::charge_all(MoleculeConstraint::ChargeSum {
        atoms: None,
        sum: NumForm::Lit(2),
    })]
    #[case::charge_empty(MoleculeConstraint::ChargeSum {
        atoms: Some(vec![]),
        sum: NumForm::Lit(0),
    })]
    #[case::bond_subset(MoleculeConstraint::BondOrderSum {
        bonds: Some(vec![BondId(0), BondId(1)]),
        sum: NumForm::Lit(3),
    })]
    #[case::bond_all(MoleculeConstraint::BondOrderSum {
        bonds: None,
        sum: NumForm::Lit(4),
    })]
    #[case::bond_empty(MoleculeConstraint::BondOrderSum {
        bonds: Some(vec![]),
        sum: NumForm::Lit(0),
    })]
    #[case::coupling_vacuous(MoleculeConstraint::UnpairedElectronCoupling {
        atoms: None,
        unpaired_electrons: UnpairedElectronsForm::default(),
    })]
    #[case::connected_subset(MoleculeConstraint::Connected {
        atoms: Some(vec![AtomId(0), AtomId(2)]),
    })]
    #[case::connected_empty(MoleculeConstraint::Connected {
        atoms: Some(vec![]),
    })]
    #[case::connected_singleton(MoleculeConstraint::Connected {
        atoms: Some(vec![AtomId(3)]),
    })]
    fn test_molecule_constraint_validator_validate(
        aggregate_molecule: Molecule,
        #[case] constraint: MoleculeConstraint,
    ) {
        assert_eq!(
            MoleculeConstraintInvariantsValidator.validate(
                &aggregate_molecule,
                &constraint,
                CONFIG,
            ),
            Ok(Solution::Determined(())),
        );
    }

    #[rstest]
    #[case::charge(MoleculeConstraint::ChargeSum {
        atoms: None,
        sum: NumForm::Lit(0),
    })]
    #[case::bond_order(MoleculeConstraint::BondOrderSum {
        bonds: None,
        sum: NumForm::Lit(0),
    })]
    #[case::connected_all(MoleculeConstraint::Connected { atoms: None })]
    #[case::connected_subset(MoleculeConstraint::Connected {
        atoms: Some(vec![]),
    })]
    fn test_molecule_constraint_validator_validate_empty(#[case] constraint: MoleculeConstraint) {
        assert_eq!(
            MoleculeConstraintInvariantsValidator.validate(
                &Molecule::default(),
                &constraint,
                CONFIG,
            ),
            Ok(Solution::Determined(())),
        );
    }

    #[rstest]
    #[case::charge(
        r#"{:atoms ["C#c+" "C"] :bonds []}"#,
        MoleculeConstraint::ChargeSum {
            atoms: None,
            sum: NumForm::Lit(1),
        },
    )]
    #[case::bond_order(
        r#"{:atoms ["C" "C"] :bonds [[0 1 "*"]]}"#,
        MoleculeConstraint::BondOrderSum {
            bonds: None,
            sum: NumForm::Lit(1),
        },
    )]
    #[case::coupling_literal(
        r#"{:atoms ["C#u0#s1"] :bonds []}"#,
        MoleculeConstraint::UnpairedElectronCoupling {
            atoms: None,
            unpaired_electrons: UnpairedElectronsForm::from((0_u8, 1_u8)),
        },
    )]
    #[case::coupling_partial(
        r#"{:atoms ["C#u0#s1"] :bonds []}"#,
        MoleculeConstraint::UnpairedElectronCoupling {
            atoms: None,
            unpaired_electrons: UnpairedElectronsForm {
                count: NumForm::Lit(0),
                multiplicity: NumForm::Undetermined,
            },
        },
    )]
    fn test_molecule_constraint_validator_validate_partial(
        #[case] input: &str,
        #[case] constraint: MoleculeConstraint,
    ) {
        let molecule = mol_dsl!(input);

        assert_eq!(
            MoleculeConstraintInvariantsValidator.validate(&molecule, &constraint, CONFIG,),
            Ok(Solution::Underdetermined(())),
        );
    }

    #[rstest]
    #[case::charge_subset(MoleculeConstraint::ChargeSum {
        atoms: Some(vec![AtomId(0), AtomId(1)]),
        sum: NumForm::Lit(1),
    })]
    #[case::charge_all(MoleculeConstraint::ChargeSum {
        atoms: None,
        sum: NumForm::Lit(0),
    })]
    #[case::bond_subset(MoleculeConstraint::BondOrderSum {
        bonds: Some(vec![BondId(0), BondId(1)]),
        sum: NumForm::Lit(2),
    })]
    #[case::bond_all(MoleculeConstraint::BondOrderSum {
        bonds: None,
        sum: NumForm::Lit(3),
    })]
    #[case::connected_subset(MoleculeConstraint::Connected {
        atoms: Some(vec![AtomId(0), AtomId(3)]),
    })]
    #[case::connected_all(MoleculeConstraint::Connected { atoms: None })]
    fn test_molecule_constraint_validator_validate_contradiction(
        aggregate_molecule: Molecule,
        #[case] constraint: MoleculeConstraint,
    ) {
        assert_eq!(
            MoleculeConstraintInvariantsValidator.validate(
                &aggregate_molecule,
                &constraint,
                CONFIG,
            ),
            Ok(Solution::Contradictory(
                MoleculeConstraintInvariantsContradiction {
                    constraint: constraint.clone(),
                },
            )),
        );
    }

    #[rstest]
    #[case::charge(
        MoleculeConstraint::ChargeSum {
            atoms: Some(vec![AtomId(99)]),
            sum: NumForm::Undetermined,
        },
        Entity::Atom(AtomId(99)),
    )]
    #[case::coupling(
        MoleculeConstraint::UnpairedElectronCoupling {
            atoms: Some(vec![AtomId(99)]),
            unpaired_electrons: UnpairedElectronsForm::default(),
        },
        Entity::Atom(AtomId(99)),
    )]
    #[case::bond_order(
        MoleculeConstraint::BondOrderSum {
            bonds: Some(vec![BondId(99)]),
            sum: NumForm::Undetermined,
        },
        Entity::Bond(BondId(99)),
    )]
    #[case::connected(
        MoleculeConstraint::Connected {
            atoms: Some(vec![AtomId(99)]),
        },
        Entity::Atom(AtomId(99)),
    )]
    fn test_molecule_constraint_validator_validate_error(
        aggregate_molecule: Molecule,
        #[case] constraint: MoleculeConstraint,
        #[case] entity: Entity,
    ) {
        assert_eq!(
            MoleculeConstraintInvariantsValidator.validate(
                &aggregate_molecule,
                &constraint,
                CONFIG,
            ),
            Err(ConstraintInvariantsError::InvalidReference { entity }),
        );
    }
}
