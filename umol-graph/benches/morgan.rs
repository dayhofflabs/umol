...
fn smiles_parsing_benchmark(c: &mut Criterion) {
    ...
    c.bench_function(&format!("smiles_parse/{corpus_size}"), |b| {
        b.iter(|| {
            for s in &smiles_list {
...
fn morgan_benchmark(c: &mut Criterion) {
    ...
    group.bench_function(BenchmarkId::new("direct", corpus_size), |b| {
        b.iter(|| {
            for ast in &asts {
    ...
    group.bench_function(BenchmarkId::new("view", corpus_size), |b| {
        b.iter(|| {
    ...
    group.bench_function(BenchmarkId::new("view_with_build", corpus_size), |b| {
        b.iter(|| {
            for ast in &asts {
    ...
    group.bench_function(BenchmarkId::new("view_opt", corpus_size), |b| {
        b.iter(|| {
    ...
    group.bench_function(BenchmarkId::new("view_opt_with_build", corpus_size), |b| {
        b.iter(|| {
            for ast in &asts {
...
criterion_group!(
    benches,
    smiles_parsing_benchmark,
    morgan_benchmark,
    morgan_ecfp6_benchmark
);
